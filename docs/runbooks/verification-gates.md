...
| host-tcb | `scripts/check_host_tcb.py` | 8/8 | admissions stay false; ConcurrentDb OS parks pinned; PCT chain-3 is a plant |
| stdenv-allowlist | `scripts/check_stdenv_allowlist.py` | 2/2 | production `StdEnv` islands pinned |
| trampoline-glue | `scripts/check_trampoline_glue.py` | 4/4 | trampoline data-fate `if`s pinned (leftover ≠ theorem) |
