...
- [x] **P1.4** Extração `db.rs` estágio 1+2 — estágio 1: fingerprints dourados do open/recovery. Estágio 2 (2026-09-15): o rustc-linked open/put vive em `db_open_kernel.rs` / `db_put_kernel.rs` (e ConcurrentDb em `concurrent_open_kernel.rs` / `concurrent_put_kernel.rs`), incluídos no trampolim; Aeneas extrai os planos totais que esses ficheiros `match` (`put_handler_plan`, `open_wal_head_plan`). `db_rs_extracted=true` **não** é dump das 22k+9k linhas (I/O/Env ficam no trampolim). Freeze exige os 4 ficheiros + defs Lean + match nos handlers.
...
| P1.4 | p1 | db.rs estágio 1+2: caracterização + split open/put | done — dourado travado; stage 2 split+plans, `db_rs_extracted=true` | fingerprints + `db_put_kernel.rs`/`db_open_kernel.rs` | 2026-09-15 |
...
- `db_rs_extracted=true` sem os ficheiros split + Lean `put_handler_plan`/`open_wal_head_plan` + match nos handlers (isso seria o dump mentiroso das 22k linhas).
