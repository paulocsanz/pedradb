...
- [x] **P0.7** onda de enrollment (8 kernels sem rota + ~53 pub fns fora da superfície): kernel+twin+script no padrão dos 73 — fan-out, converge com 0221 P0.6 — status: done (8/8: client_axis, group_window, leftover_page, scan_readahead, write_cycle, durability_spine, ratio_curve, product_crown)
...
- [x] **P1.2** pisos do `sel4_gap --gate` ligados ao CI (regredir eixo = red) — status: done (`sel4_gap_floors.json` congelado no live; `--gate` GREEN; `--selftest` 2/2; job `sel4-gap-floor` no verification-gates.yml)
