...
- [x] **P1.3** `deps_mvcc_latest` e `deps_scan` ≥ 0.5 × Rocks FF da run — status: `done` (cache last+count + seek O(log N) no get: combinada ycsb+deps MVCC **0.91×** / scan **1.38×** vs FF; 11/11 ≤2×)
