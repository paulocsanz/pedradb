...
6. **P2.1:** cadência final do bloco — txn ×6 (`discard_cut`,
   ...
   — 2/7 `done`: `leftover_txn_is_aborted_fate_iff`
   (StoreTxn.lean; TX preparada sobrante sem coordenador é
   ABORTADA no recovery — fate exatamente `true`; o as-is mantinha
   intents vivos, Conflito imortal + reuso de id), cap 32→31,
   floor_atom 92→93, floor_extract 186→185; planta DST verde
   (`leftover_txn_is_aborted_on_live_reopen_is_not_ok`, no mesmo
   lote paralelo)

   — 3/7 `done`: `next_txn_id_after_fate_iff`
   (StoreTxn.lean; o próximo txn id é EXATAMENTE
   `max(max_seen+1, 1)` — nunca reusa id em disco/contador
   durável; o as-is reiniciava em 1, reuso de id), cap 31→30,
   ...
   — 5/7 `done`: `reserve_si_gen_fate_iff`
   (StoreTxn.lean; reservar uma SI generation AVANÇA o contador e
   devolve o novo valor — gens pendentes distintas; o as-is
   devolvia `current+1` sem avançar o contador, colisão de gen),
   cap 29→28, floor_atom 95→96, floor_extract 183→182; planta DST
   verde (`reserve_si_gen_on_live_queued_is_not_ok`, no mesmo lote
   paralelo)

   — 6/7 `done`: `unreserve_si_gen_fate_iff`
   (StoreTxn.lean; desfaz a reserva EXATAMENTE quando nada
   reservou depois (`stamped>0 ∧ current==stamped` → `stamped−1`,
   senão `current`); o as-is rolava o contador para trás mesmo
   com gen já avançada — reemite a mesma gen), cap 28→27,
