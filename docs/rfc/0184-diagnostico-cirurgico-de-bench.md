...
- [x] **P2.61** `nebula_insert_edge_mc4` (edge WriteBatch ingest, 4
      clients) in `COMPARE_SHAPES`, `BALANCE_SHAPES`, and
      `run_nebula_insert_clients`. Test
      `rfc0184_nebula_insert_edge_mc4_in_compare`. Darwin DIAG
      `ratio=1.091` (33.2 k / 30.4 k QPS). Same-class async
      (`peer_policy=rocks-default`, `sync=false`). p50 121 vs 100 µs
...
- [x] **P2.62** `solana_shred_append_mc4` (WriteBatch of 16 shreds, 4
      ...
      — status: `done`
- [x] **P2.63** `arango_doc_crud_mc4` (50% get / 30% put / 20% scan, 4
      clients) in `COMPARE_SHAPES`, `BALANCE_SHAPES`, and
      `run_arango_crud_clients`. Test
      `rfc0184_arango_doc_crud_mc4_in_compare`. Darwin DIAG
      `ratio=0.445` (239 k / 537 k QPS). Same-class async
      (`peer_policy=rocks-default`, `sync=false`). p50 5.1 vs 2.1 µs.
      Rocks 537 k mix-ops is not collapsed. Named loss. Not Linux
      cartaz. — status: `done`
- [x] **P2.64** `kvrocks_pipelined_set_mc4` (WriteBatch of `batch` SETs, 4
      clients) in `COMPARE_SHAPES`, `BALANCE_SHAPES`, and
      `run_kvrocks_pipelined_clients`. Test
      `rfc0184_kvrocks_pipelined_set_mc4_in_compare`. Darwin DIAG
      `ratio=0.807` (29.9 k / 37.0 k QPS). Same-class async
...
- [x] **P2.65** `rockstore_widecol_rw_mc4` (50% put / 50% col-prefix scan, 4
      ...
      cartaz. — status: `done`
- [x] **P2.66** `kvrocks_blob_set_mc4` (16 KiB BlobDB-sized SET, 4
      clients) in `COMPARE_SHAPES`, `BALANCE_SHAPES`, and
      `run_kvrocks_blob_clients`. Test
      `rfc0184_kvrocks_blob_set_mc4_in_compare`. Darwin DIAG
      `ratio=0.766` (46.3 k / 60.5 k QPS). Same-class async
