...
- [x] **P0.55** `wait_in_flight_to_queue` limitado a 4096 `spin_loop`
...
- [x] **P0.62** Unpinned overwrite **supersedes** the live memtable slot
      ...
      status: `done`
- [x] **P0.66** Tail idx shards raw one-slash keys (`c/`, `ycsb/`, `u/`)
      so overwrite inserts do not share the empty-prefix BTree with the
      YCSB seed. Multi-slash stays empty (F220 `["d/m/", "d/m0")`).
...
- [x] **P0.70** 1c host worker checkpoints WAL (flush live mem + rotate)
      when WAL ≥ 2× write-buffer. Seed never sits 200 ms idle so the log
      otherwise grows to the whole keyspace (25M ≈ 3 GiB on a 4 GiB box).
