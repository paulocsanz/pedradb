...
- [x] **P1.2** TCP 3-process — status: `done` (`l28_real_tcp_drop_st`)
...
| P1.2 | p1 | TCP 3-process | done | l28_real_tcp_drop_st | 2026-08-30 |
...
- **Tests**
  ...
  - P1.2 `l28_real_tcp_drop_st`: seed `0x0148_1E28` twice (default durability, not `--remove-member`); fingerprints match; `sth=1`; production TCP ctor of n1 (3 still in `ids`, 3 not in local `nodes`) plants `sent_through` for 2 and 3; oob `remove_member(3)` keeps 2 and drops 3. 0147 joint slot drop is **not** this tooth. Exit via `l28_tcp_sth_ok`. AS-IS keeps `sent_through`. Runs on Darwin. Does not submit io_uring SQEs.
