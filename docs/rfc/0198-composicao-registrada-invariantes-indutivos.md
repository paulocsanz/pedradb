...
- Débito de script restante: `unpaid_script=2/17` — `finish_group_off_lock`
  ...
  `crates/pedradb-core/src/concurrent_kernel.rs`, arquivo NÃO-commitado da sessão
...
- [x] **P0.2** Segundo close registrado: `occ_batch_plan ∘ occ_conflict`
  (glue de `validate_occ_batch`/`lone_commit`) — iff ∀ em
  `GroupCommit.lean`; `floor_close` 2→3 no mesmo commit — status: `done`
  (theorem `occ_batch_plan_member_fate_iff`: ∃-bind do callee
  `occ_conflict` no corpo do loop, fate por membro decidido exatamente
  pela cadeia — TooOld pela flag, Conflict/Ok pela resposta ok do callee;
  floor 247/3, residuals close 4; aterrissado após o commit 4df272d5 da
  sessão paralela assentar os compartilhados — monitor)
...
- [x] **P2.2** Cadência cap continua: cada novo atom df desce
  ...
  status: `done` (primeira descida do RFC: `occ_snap_uses_published`
  promovido a atom — `occ_snap_uses_published_ok_iff_inflight` em
  `Flush.lean`, molde passo-30; atom row + catalog flip (data_fate
  removido do par, three_teeth mantém os dentes) + floor_extract
  247→246 + floor_atom 31→32 + cap 100→99 + residuals NO MESMO COMMIT;
  planta `occ_snap_uses_published_on_live_inflight_is_not_ok` passa; a
  cadência continua pelos próximos RFCs)
...
| P0.2 | p0 | Close de glue registrado: occ_batch_plan∘occ_conflict | done | occ_batch_plan_member_fate_iff | 2026-09-11 |
