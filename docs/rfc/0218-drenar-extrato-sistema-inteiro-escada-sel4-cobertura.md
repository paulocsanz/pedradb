...
9. **P2.2** raft/dcs/l28/scale/bloom/probe ×21: `apply_step`,
   ...
   status: `todo`
   (21/21: `bloom_header_fate_iff`, `insert_fate_iff`,
   ...
   `happy_hot_bps_fate_iff`, `warm_cap_bytes_fate_iff`,
   `predict_get_ns_fate_iff`, `scale_forecast_fate_iff`,
   `l28_durability_ok_fate_iff`,
   `l28_tcp_napply_retry_admitted_fate_iff`,
   `recover_last_applied_fate_iff` `apply_advance_fate_iff` `ae_f16_safe_fate_iff` `dcs_apply_should_advance_result_fate_iff` `dcs_apply_should_advance_fate_iff` `next_lease_id_after_fate_iff` e `lease_table_expired_fate_iff` feitos 2026-09-13)
