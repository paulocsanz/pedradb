...
- [x] **P0.1** Runner PCT in-tree (`pedradb-dst` ou `pedradb-world`) sobre `ConcurrentDb`: 2–4 tasks, yield nos pontos de lock/group; **sem** `dst_core` obrigatório (PCT mínimo copiado, ~Fig. 7) — status: `done` (`crates/pedradb-core/src/pct_hooks_kernel.rs` turnstile feature `pct` + hooks `op_entry`/`lead_write_lock`/`follower_wait` no commit path real; `crates/pedradb-world/src/pct_concurrent_kernel.rs` `run_pcts` 3 tasks; teste `pct_runner_drives_real_concurrentdb`: mesmo seed 2× ⇒ mesmo `schedule_hash`, 36/36 puts visíveis, durable no reopen)  
