...
- `persist_hard_db` tem dois caminhos vivos no store. O do voto está congelado desde o 0152 (par `grant_persist`: `vote_kernel::grant_after_persist` + planta `grant_after_persist_on_live_queued_is_not_ok`). O do **termo** não: `durable_become_follower_if_newer` (`crates/pedradb-store/src/lib_kernel.rs:2605`, F125/F127) é cola inline em 6 sítios inbound (`on_request_vote`, `on_append_entries`, replies, …).
