...
- [ ] **P1.1** `deps_apply_batch_mc4` e `deps_raftlog_mc4` ≥ 2.0 vs default da run — status: `doing` (tail index + no spill/stage on Ok; host stages after burst; L0 uncompressed. Remesura 16-shape espera caixa quieta. parkfold2 still best official Pedra 4.3 k)
...
| P1.1 | p1 | apply/raftlog MC ≥ 2× | doing | no BTree/lz4 on apply Ok; remesura pending (load) | 2026-08-17 |
