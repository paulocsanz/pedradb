...
- [ ] **P1.1** `deps_apply_batch_mc4` e `deps_raftlog_mc4` ≥ 2.0 vs default da run — status: `doing` (WAL write syscall off write lock; remesura 16-shape espera caixa quieta. parkfold2 still best Pedra 4.3 k)
...
| P1.1 | p1 | apply/raftlog MC ≥ 2× | doing | WAL write off lock; remesura pending (load) | 2026-08-17 |
