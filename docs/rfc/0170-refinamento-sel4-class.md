...
- [x] **P0.3** Extract Aeneas de `crates/pedradb-core/src/prefix_kernel.rs` (já close): stamp `SOURCE.prefix`, Lean `prefix_exclusive_end_matches_spec` sem `sorry`, dente as-is no extract — status: `done`
...
- [x] **P1.3** Família HTTP auth/form (`bearer`, `is_bearer_scheme`, `is_non_bearer_auth_scheme`, `normalize_http_method`, `authorization_matches`, `form_plus`, `query_values_conflict`, `query_part_is_bare_name`) — status: `done` (dentes que faltavam pagos 2026-09-09, fire 791: `ascii_lower_as_is`/`ascii_upper_as_is` + teste `ascii_fold_discriminates_as_is` e `is_non_bearer_auth_scheme_as_is` + teste `non_bearer_scheme_gate` no `auth_kernel.rs`; re-extract `aeneas_auth.sh` stamp `477d9ac4`, `AuthKernel.lean` 0 sorry; formal 172→166; fire 792: `from_hex_as_is` + `plus_before_percent_as_is` + testes `hex_letters_decode_as_is_does_not` e `plus_order_flag_discriminates_as_is` no `form_kernel.rs`, re-extract `aeneas_form.sh` stamp `ef40905b`, formal 166→162; fire 793: `authorization_matches_as_is` (loop manuscrito no patcher, patched ×7) + 5 live-plants no `lib.rs` (`invalid_cl_as_zero`/`content_length_repeat_ok`/`short_body_vs_cl`/`host_authority_mismatch`/`authorization_matches` — raw TcpStream + half-close contra `KvServer` vivo, 400 fail-closed em todos; http 74/74 serial; stamp `9a3cd04b`; formal 162→155 — dívida http three-teeth zerada nos fires 790–793, 179→155)
