...
| group / catch-up | `crates/pedradb-core/src/concurrent_kernel.rs` `WriteGroup` |
| count / SST cursor | `crates/pedradb-core/src/db_kernel.rs` `count_visible`, `SstCountCursor` |
| encode WAL | `crates/pedradb-core/src/batch_kernel.rs` `encode_ops`; `wal/mod.rs` `append_write_ops` |
