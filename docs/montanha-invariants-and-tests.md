...
API: [`StoreCluster::put_batch`](../../crates/pedradb-store/src/lib_kernel.rs) — one raft log entry `RangeEntry::Batch`, apply via PedraDB `apply_batch`.
...
| I-TX-6 fail ⇒ restore preimage (not delete) | `revert_restores_preimage_on_partial_commit` | pedradb-store |
| I-TX-7 crash after prepare ⇒ no immortal intents | `crash_after_prepare_does_not_immortalize_intents` | pedradb-store |
| I-SI-1 snapshot + OCC survive reopen | `snapshot_isolation_survives_reopen` | pedradb-store |
| I-LAYER-1 SafeAllocator exact after crash-retry | `safe_allocator_exact_after_crash_retry` | pedradb-store |
| I-LAYER-2 NaiveAllocator leaks (fonte 1021) | `naive_allocator_leaks_name_on_crash_retry` | pedradb-store |
