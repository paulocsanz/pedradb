...
**Open (RFC-0033):** P0.1/P0.2 shipped — `Db::last_under_prefix` (mem BTree reverse + SST `point_at` from the back, confirm via `lookup`, shrinking `before` so a tombstone cannot leak or stall). `latest_cf` / `last_key_with_prefix` use it. Per-layer scan cap **not** shipped (would hide live keys after a deleted prefix — G2). P0.3 = lazy block pull. Ver [RFC-0033](rfc/0033-mvcc-scan-2x.md).
