...
**Open (RFC-0035):** P0 done. Finding: MVCC p50 = `latest` 39 µs + `get` 1 KB **38 µs**; 79% mem-hit; 21% SST (3 files). Get sozinho já é 7× o op inteiro do Rocks (5.4 µs) — **2× é impossível até o get cair ~6 µs**. Scan: 2.57 decode/op, cache 19%. P1.1 = get_cf 1 KB. Ver [RFC-0035](rfc/0035-mvcc-scan-2x-measure-first.md) + [finding](../findings/rfc0035-p0-ranked-bottlenecks.md).
