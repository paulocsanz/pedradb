...
**Open (RFC-0032):** 2× **nesta tabela** TiKV-mix. Gate oficial = vs Rocks **F_FULLFSYNC** ≥ 0.5 (escritas A/B/F/apply já passam). P0 = iterador janela + latest por prefixo (MVCC/scan). P1 = point-get C (~25×). Sem floor 2× de escrita vs fdatasync (G1). Ver [RFC-0032](rfc/0032-tikv-mix-2x-budget.md).
