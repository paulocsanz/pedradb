...
**Open (RFC-0035):** P0+P1.1 shipped. Get 1 KB é **inline** (vlog=0). `lookup` salta SST se a mem já tem o ponto; latest+get num lock. p50 get ainda ~36 µs — **2× não**. P1.2 = explicar esses 36 µs (não vlog). Scan inalterado (~27×). Ver [RFC-0035](rfc/0035-mvcc-scan-2x-measure-first.md).
