...
**Open (RFC-0033):** P0.1–P0.3 shipped. `last_under_prefix` for MVCC latest; scan pulls SST blocks lazily and stops when `emitted == limit` (range tombstones collected first — no per-layer cap). `BlockCache` wired on this path. Residual vs 2× floor is still the number of overlapping L0 files per seek. Ver [RFC-0033](rfc/0033-mvcc-scan-2x.md).
**Open (RFC-0034):** teto **1.1×** (`Pedra / Rocks_F_FULLFSYNC ≥ 0.91`) em **todos** os 11 shapes da tabela. Sem 1.1× de escrita vs fdatasync (G1). P0 = remesura 4096/2000 + gate nos já-verdes (A/F/overwrite). Ver [RFC-0034](rfc/0034-rocks-parity-1.1x-all-shapes.md).
