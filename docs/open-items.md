...
**Open (RFC-0031):** orçamento de paridade ≤10× do RocksDB por shape. **P0.1 shipped:** CHANGELOG debounce inline (`PEDRA_CHANGELOG_INTERVAL` default 64; persist em flush/close/WAL-rotate/checkpoint; G1–G8 intactos; adversarial rocksdb-compat re-verde sem editar asserção). Escritas ~4× (ycsb_a 94→390; seed 39.7s→6.5s). Residual = 1× WAL `sync_all`/`F_FULLFSYNC` (~3.8 ms/put; `interval=0` ≈ 64) — **não** relaxar para `sync_data`. Floor 0.1 continua report-only (P0.2). P1 = iterador janela. Ver [RFC-0031](rfc/0031-rocks-parity-10x-budget.md).
