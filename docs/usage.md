...
| Older than the window | Auto-compact **archives first** (CRC'd segments under `history/`), then drops — **LSM caveat (measured, `findings/rfc0046-sizing/`)**: the horizon floor lags the L0-merge inputs (versions age after reaching L1, which the L0 path never rewrites), so on the default auto path the LSM keeps aged versions like F20 and disk = live set + archive cap *on top of* the retained LSM. Bounded total disk today requires `set_auto_reclaim(true)`; the horizon-driven old-level rewrite is RFC-0046 P0.5 (open) |
