...
| Insert memtable → flush (amortizado) | writes sob o cap | k writes ⇒ work total de flush ≤ c·k (crédito; N concreto) | **count** (P1.2: `memtable_flush_amortized` — paid ≤ written + initial, invariante `run_paid_plus_mem_le` (cada byte pago ≤ 1×); pontes `auto_flush_due_fires_iff`/`auto_flush_due_hold_under_limit` no extract `FlushKernel` (dispara iff armed ∧ limit ≤ mem); `FlushAmortCount.lean`; twin `tests/flush_amort_count.rs` — schedule sim dirigindo o gate real `auto_flush_due`) |
...
| Bloom probe (`catalog:bloom_may_contain`) | k hashes da política | probes = k por consulta (loop extraído) | todo (P1.2/P1.4 wave) |
