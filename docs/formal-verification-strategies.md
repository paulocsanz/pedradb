...
| P1 | Bloom filter T1–T4 (beyond F166 header) | 2–3 | **In progress 2026-08-15 (RFC-0030):** production `insert`/`may_contain` — no false negatives, roundtrip, in-bounds under F166, inactive never rejects. Exhaustive domain + teeth + Verus model-domain twin (7 verified) landed. Kani on compiled production (T4 green; T1–T3 bounds reduced after a 1 h/12 GB CBMC stall). Aeneas extract of production `bloom.rs` + Lean T4 accepted; loop-level T1 not claimed. |
