...
| L28 | Swarm / buggify no DST do store (fidelidade FDB §4) | `MEASURE` | ficha | R043 | p. 8–9; já há `cluster_dst_*`. Programa: [RFC-0050](../docs/rfc/0050-world-in-tree-fdb-determinism.md) P2.2 **entregue** (`swarm_l28_mask_matrix`: n∈{3,5}×3 máscaras≠all×4 seeds = 24/24 clean em 2026-08-23). Gate: bug de cluster REAL reproduzido por seed no sim **in-tree** — ainda não atingido; swarm limpo até aqui |
...
| L45 | DST recovery sob Miri / ASan FFI / mesmo seed TCG=`trace_hash` — caixas empilhadas no *ciclo*, não num interpretador | `MEASURE` | rfc | R043 | Paper §4: sim não testa FS/OS nem código fora de Flow. Programa: [RFC-0052](../docs/rfc/0052-dst-inside-boxes.md). **REFUSE:** Miri-in-TCG, TCG-bench (finding 2026-08-22, distorção 11×), ASan+Miri no mesmo processo |
| L46 | Sanduíche IronFleet em Rust de produção (anos): TCB + crash-dictionary + 2ª máquina; **não** Dafny/`db.rs` ∀ | `MEASURE` | rfc | — | IronFleet SOSP’15 §7 ~3.7 py; VeriBetrKV crash=IOSystem, 8× Rocks **recusado** como trade. Programa: [RFC-0053](../docs/rfc/0053-ironfleet-years.md). Gate Y1: P40 iff no extract Aeneas + spec crash com dentes DST. **Não** claim “Pedra verificado” |
