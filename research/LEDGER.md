...
| L45 | DST recovery sob Miri / ASan FFI / mesmo seed TCG=`trace_hash` — caixas empilhadas no *ciclo*, não num interpretador | `MEASURE` | rfc | R043 | Paper §4: sim não testa FS/OS nem código fora de Flow. Programa: [RFC-0052](../docs/rfc/0052-dst-inside-boxes.md). **P0+P1+P2.2 (2026-08-24):** `miri-dst-smoke`, `tsan-box`, `capi-asan-harness`; P1.3 Tree Borrows `scripts/miri_dst_tree_borrows.sh` SB=TB (not required CI); sem guest `scripts/tcg_guest_status.sh` → `C2.2=residual_no_guest`. **P2.1+P2.3 (2026-08-24):** `tcg_world_smoke.sh` native=guest hash `61f8a02125b3c69e`; `tcg_world_smoke_detio.sh` PRELOAD `libdet_io.so` 215 fsync drops, same hash. **REFUSE:** Miri-in-TCG, TCG-bench (finding 2026-08-22, distorção 11×), ASan+Miri no mesmo processo |
| L46 | Sanduíche IronFleet em Rust de produção (anos): TCB + crash-dictionary + 2ª máquina; **não** Dafny/`db.rs` ∀ | `SHIP` | rfc | — | IronFleet SOSP’15 §7 ~3.7 py; VeriBetrKV crash=IOSystem, 8× Rocks **recusado** como trade. Programa: [RFC-0053](../docs/rfc/0053-ironfleet-years.md) (Y1–Y3 entregues 2026-08-23) + face produto [RFC-0058](../docs/rfc/0058-verified-mode-kernel-derived-fallback.md) **P0+P1+P2 entregues** (2026-08-24): `profile_report()` = os kernels do catalog como composição declarada (machine-checked), suíte FailingEnv/World/PCT no perfil (silent_wrong=0; fence ≤ 1 escritor), `open_verified` fold/lease/dcs/compat, derivação verified=full por oráculo, CI `verified-mode`; **P2 (2026-08-24): group-commit de volta por teorema** — kernel `group_commit_kernel.rs` (Verus 14 verified/0 errors; extrato Aeneas→Lean `GroupCommit.lean` sem sorry; atomicidade de grupo + first-committer-wins + fence=max), perfil `verified-v2` (merge decidido pelo kernel, catch-up 0, bypass async; report ON), ring io_uring fora com contrato publicado, `PEDRA_VERIFIED=1` no CLI como linha de produto; extração total segue REFUSE. Gate Y1: P40 iff no extract Aeneas + spec crash com dentes DST. **Não** claim “Pedra verificado” |
...
| L49 | At-rest CRC scrub + World BitFlip | `SHIP` | rfc | — | [RFC-0060](../docs/rfc/0060-field-and-hardware-residuals.md) P0–P1: `verify_at_rest` / `pedra verify`; BitFlip fail-closed (`flush`→XOR→`verify_at_rest` on the live Env→reopen). **Does not prove media.** Distinct from L22 (Percolator). |
| L50 | Hardware residual (ECC / bit-rot not on a read path / CPU) | `OPEN` | rfc | — | CRC+scrub cover SST/vlog/WAL/MANIFEST **when walked**. `CURRENT` and `*.tmp` are named holes. ECC is the drive's job. TCG/det_io (RFC-0052 P2) is the remaining "is the model the hardware?" box — not promised here. Distinct from L23 (timestamp oracle). |
