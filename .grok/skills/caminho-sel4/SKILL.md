...
- Last fire: worked — RFC-0232 P0.1 trampoline-glue freeze (`check_trampoline_glue.py` 4/4); leftover=2 (`refuse_publish`, unpublished-below-floor); 0229 P0 host-tcb still GREEN (admissions false)
- Why: leftover_next none (Env glue remains); Ho et al. 2026 arXiv:2609.15648 — platform/runtime/intrinsics stay TCB; Pedra analog is trampoline Env `if`s, not dump. Isolated `get_live` still uncataloged.
- Result corrections duráveis: `scripts/ratchet/trampoline_glue_ifs.tsv` + job `trampoline-glue`; plant `rfc0232_trampoline_glue_leftover_ifs_inventoried`
- Next fire MUST: RFC-0232 P0.2 `pedra_refines` unfold `write_group_wait_grant` × `lock_alphabet_linearizes_n2` (A1 stays 1), or catalog Isolated `get_live`. OS flags stay false.
- Foundational Mathematical Doctrines (RFC-0278):
  1. LSM Inductive Bisimulation: Never claim verified storage without an explicit abstraction function α(LSM) → Map(K → V) proving snapshot isolation and compaction key-shadowing preservation.
  2. Crash-Refinement: Never rely solely on empirical fault injection; recovery must refine to an exact monotonic prefix of acked transactions without phantom records.
  3. Universalization & Anti-Vacuity: Never write Lean theorems on concrete constants (`96#u64`); theorems must be universally quantified (∀), with satisfiable non-vacuous premises.
  4. Concurrency Soundness (Loom + Miri): Pair bounded Loom model checking with Miri Tree-Borrows and Data Race detection on concrete execution traces.
  5. Bounded Resources & Stack: Write, flush, and recovery paths must have verified O(1) dynamic allocations and bounded recursion depth (≤ MAX_LEVELS + 2).
- Forbidden this fire: mint toy twin verus; stamp over sorry; wrap plumbing theorems; touch /tmp/db.rs.pipeline-ORIGIN; git checkout/stash db.rs/concurrent.rs; touch frozen Montanha files; leftover wrap em pedradb-store/montanha; re-pin Aeneas/charon sem widen medido sem sorry; editar o shim de produção (usar clone /tmp com #[path] absolutos); flip media_durable / forall_schedules / lock_interleavings; Iris/hashmap heap
- Deeper: Aeneas Lean recipes: direct bodies `rfl`; decide/if chains `unfold; simp`; saturating chains `have h := by native_decide; simp [h, lift, Ord.max.default, Ord.max_body, PartialOrdU64.lt]`; theorem headers `open Aeneas Aeneas.Std Result`; Result-typed eq/str fns are extract axioms — conditional theorems, not native_decide; U64 `<` is Prop — use `decide` against extracted Bool
