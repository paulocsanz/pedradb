...
- Last fire: worked (`4190ebbc` delete lease_kernel.rs Verus cartoon; rustc `lease_live` is the term)
- Why: leftover_next_first wal_recover; Verus stand-in billed as last-wins of rustc recover
- This fire MUST land: delete the `wal/recover_kernel.rs` `verus!` stand-in; rustc types stay; Aeneas of those types is the term
...
- Deeper: Search leftover_next=cartoon remaining (skip store); open recover_kernel.rs; named cargo; `aeneas_*.sh` / lake
