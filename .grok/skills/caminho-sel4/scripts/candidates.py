...
PRODUCT_RFC = RFC_DIR / "0227-prova-de-produto-put-get-recover-replica.md"
PRODUCT_RFC_0191 = RFC_DIR / "0191-pacote-garantias-produto.md"
...
def rfc0191_open() -> list[str]:
    rfc = PRODUCT_RFC if PRODUCT_RFC.is_file() else PRODUCT_RFC_0191
    if not rfc.is_file():
        return ["P0.1"]
...
def product_board() -> tuple[int, str | None]:
    """RFC-0227 (else 0191) product rows. Missing ratchet = P0.1 unpaid."""
    print("== product (RFC-0227: D1/R1/T1/C1 over rustc handler) ==")
    opens = rfc0191_open()
...
def print_leftover_next(
    ...
    if unpaid_product and product_next and product_next not in trampoline_ids:
        print(
            f"  leftover_next product remaining RFC-0227 {product_next}; "
            "handler subject close; Isolated get walk; pedra_refines Rel; "
            "layer model→atom→close only up; skip Montanha"
        )
        print(f"  leftover_next_first RFC-0227 {product_next}")
        return
