...
## A verificação move com o código de engine (mesmo fire, nunca depois)

Fire que adiciona ou muda fn de kernel (`*_kernel.rs`) atualiza a
verificação formal **no mesmo turno** — matrícula RFC-0222 P0.7:

- linha em `scripts/formal/catalog.json` (`twin_kind` atom/close,
  `single_artifact`, fn `as_is`, `dst_plant` = o teste nomeado,
  `atom_reason` datado) e a fn **sai** do
  `glue.kernel_fn_allowlist` em `scripts/formal/residuals.json`
  (baseline só encolhe);
- Lean: re-rodar o `scripts/aeneas_<kernel>.sh` do kernel, teorema em
  `formal/aeneas/lean/<X>.lean`, registro em
  `scripts/ratchet/close_proofs.tsv`;
- ledger e pisos no mesmo commit: marcadores de
  `docs/verification-ledger.md` (total/proof/single_artifact/
  aeneas_scripts), `glue.proof_depth`/`glue.single_artifact` no
  `residuals.json`, `floor_atom` em `scripts/ratchet/proof_depth.tsv`,
  `atom_fns`/`pairs_*` em `scripts/ratchet/sel4_gap_floors.json`;
- gates antes de fechar o fire: `./scripts/pedra_formal.sh --ci`,
  `scripts/check_depth_floor.py`,
  `scripts/check_ledger_consistency.py`, `scripts/sel4_gap.py --gate`.
  Sem NOVO vermelho (vermelho pré-existente entra nomeado no journal,
  nunca escondido — o `--ci` carrega dívida conhecida do P0.8: varredura
  de clones com ~576 pares triviais `new`/`get`/`len` e 3 falhas
  pontuais em montanha-fdb-recipes/properties; não é desculpa para
  não rodar).

Modo de falha real (fire 2026-09-14): `seal_async_first_drain`
aterrissou como fn + teste nomeado mas sem linha de catálogo — ficou
estacionada no allowlist, invisível aos dentes; dívida paga 2026-09-15.
Kernel fn nova no allowlist ao fim do fire = **failed fire**, mesmo com
`ratio=` no journal.
...
6. Nunca skiplist sem `despark=1`. Nunca Darwin como cartaz. Nunca T-clone.
   Nunca HashMap no empty-prefix tail idx (ycsb_e 0.165 P0.64 / P1.3).
...
- Kernel fn landed without its catalog row / with `kernel_fn_allowlist`
  grown — verification debt in the same fire is a failed fire.
