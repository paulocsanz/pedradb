...
| `scale` | `scale_kernel.rs` | `point_get_probes_one_plus_one` / `_as_is_is_n_files` / `probes_worst_l0_trigger_via_point_get` / `worst_get_ns_l0_trigger_via_probes_worst` / `happy_get_ns_l0_best_via_point_get` |
...
| `merge` | `merge.rs` | `visible_at_deletion` / `_as_is_dente` / `user_key_in_range_unbounded` / `past_end_unbounded` / `iter_window_keep_hidden` (shim `#[path]` key+compact; `--start-from` visible_at + range + window bounds + keep; WindowKvIter is Iterator; pair `visible_at` is single_artifact: production file is the Verus term) |
