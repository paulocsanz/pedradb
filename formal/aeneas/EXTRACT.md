...
| `fail_closed` | `fail_closed.rs` | `parse_error_writes_status_true` / `reject_transfer_encoding_true` / `present_bad_int_is_error_true` / `parse_error_status_400` (`--start-from` F102 + Iterator-free F104/F105/F157/F158 + status/as-is constants; `header_break_end` emits Iterator.`position` extra field; Expect production walks are `str/pattern` holes) |
