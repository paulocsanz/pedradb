...
| `fail_closed` | `fail_closed.rs` | `parse_error_writes_status_true` / `reject_transfer_encoding_true` / `present_bad_int_is_error_true` / `parse_error_status_400` / `header_break_len_below_four` (`--start-from` F102 + Iterator-free F104/F105/F157/F158 + status/as-is + header_break; Windows Iterator extra `position` stripped; Expect production walks are `str/pattern` holes) |
