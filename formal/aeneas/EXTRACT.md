...
| `leveling` | `leveling.rs` | `level_target_bytes_l0` / `_as_is_l0` (`RUSTFLAGS=--cfg test` so Charon sees as_is; pick Iterator holes patched to index loops; Iterator extra fields stripped; pairs `leveling` and `leveling_pick` are single_artifact: production file is the Verus term) |
