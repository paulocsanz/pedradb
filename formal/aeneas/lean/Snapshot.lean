...
/-- AS-IS tooth: leftover TX meta survives the snapshot. -/
theorem snapshot_needs_txn_meta_clear_as_is_tooth :
    snapshot_needs_txn_meta_clear_as_is = ok false := by
  rfl

/-- RFC-0218 P1.3 2/11 (atom `catalog:snap_txn_clear`, entrada
    `snapshot_needs_txn_meta_clear`): restaurar snapshot SEMPRE exige
    limpar o metadado de txn — EXATAMENTE a constante citada true.
    O AS-IS é false (txn meta vaza entre snapshots — tooth
    plantado). -/
...
/-- RFC-0218 P1.3 6/11 (atom `catalog:snapshot`, entrada
