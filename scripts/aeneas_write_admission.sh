#!/usr/bin/env bash
# Extract production write_admission_kernel.rs (RFC-0170 P2.1).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/write-admission-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/write_admission_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/write_admission_kernel.llbc"
{
...
} > "$OUT/SOURCE.write_admission"
echo "ok    extract write_admission → $OUT"
