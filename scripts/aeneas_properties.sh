#!/usr/bin/env bash
# Extract production crates/pedradb-spec/src/properties_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/properties-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/properties_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/properties_kernel.llbc"
{
  echo "path=crates/pedradb-spec/src/properties_kernel.rs"
...
echo "ok    extract properties → $OUT"
