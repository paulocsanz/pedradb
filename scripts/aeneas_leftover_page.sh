#!/usr/bin/env bash
# Extract production leftover_page_kernel.rs (RFC-0222 P0.7).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/leftover-page-kernel"
...
mkdir -p "$OUT"
SRC="$ROOT/crates/pedradb-core/src/leftover_page_kernel.rs"
echo "      charon=$CHARON"
( cd "$CRATE" && "$CHARON" cargo --preset=aeneas --dest-file "$OUT/leftover_page_kernel.llbc" )
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/leftover_page_kernel.llbc"
python3 - "$OUT/lean/LeftoverPageKernel.lean" <<'PYEOF'
import sys
p = sys.argv[1]
...
PYEOF
{
  echo "path=crates/pedradb-core/src/leftover_page_kernel.rs"
...
} > "$OUT/SOURCE.leftover_page"
echo "ok    extract leftover_page → $OUT"
