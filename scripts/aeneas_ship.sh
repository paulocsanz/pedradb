#!/usr/bin/env bash
# Extract production crates/pedradb-replicate/src/ship_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/ship-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/ship_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/ship_kernel.llbc"
{
  echo "path=crates/pedradb-replicate/src/ship_kernel.rs"
