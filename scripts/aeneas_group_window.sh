#!/usr/bin/env bash
# Extract production group_window_kernel.rs (RFC-0222 P0.7).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/group-window-kernel"
...
mkdir -p "$OUT"
SRC="$ROOT/crates/pedradb-core/src/group_window_kernel.rs"
echo "      charon=$CHARON"
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/group_window_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/group_window_kernel.llbc"
python3 - "$OUT/lean/GroupWindowKernel.lean" <<'PYEOF'
import sys
p = sys.argv[1]
...
PYEOF
{
  echo "path=crates/pedradb-core/src/group_window_kernel.rs"
...
} > "$OUT/SOURCE.group_window"
echo "ok    extract group_window → $OUT"
