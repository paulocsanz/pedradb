#!/usr/bin/env bash
# Extract production ae_kernel.rs via the standalone crate.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/ae-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/ae_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/ae_kernel.llbc"
{
  echo "path=crates/pedradb-raft/src/ae_kernel.rs"
