#!/usr/bin/env bash
# Extract production crates/montanha-fdb-recipes/src/pack_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/pack-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/pack_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/pack_kernel.llbc"
{
  echo "path=crates/montanha-fdb-recipes/src/pack_kernel.rs"
