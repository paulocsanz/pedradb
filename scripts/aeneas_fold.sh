#!/usr/bin/env bash
# Extract production crates/pedradb-fold/src/fold_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/fold-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/fold_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/fold_kernel.llbc"
{
  echo "path=crates/pedradb-fold/src/fold_kernel.rs"
