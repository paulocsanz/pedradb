#!/usr/bin/env bash
# Extract production crates/pedradb-store/src/rpc_mode_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/rpc-mode-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/rpc_mode_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/rpc_mode_kernel.llbc"
{
  echo "path=crates/pedradb-store/src/rpc_mode_kernel.rs"
