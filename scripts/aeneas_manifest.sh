#!/usr/bin/env bash
# Extract production crates/pedradb-core/src/manifest_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/manifest-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/manifest_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/manifest_kernel.llbc"
{
  echo "path=crates/pedradb-core/src/manifest_kernel.rs"
