#!/usr/bin/env bash
# Extract production crates/pedradb-store/src/index_val_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/index-val-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/index_val_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/index_val_kernel.llbc"
{
  echo "path=crates/pedradb-store/src/index_val_kernel.rs"
...
} > "$OUT/SOURCE.index_val"
echo "ok    extract index_val → $OUT"
