#!/usr/bin/env bash
# Extract production bloom.rs via the standalone crate (RFC-0030 P2.1).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/bloom-kernel"
...
mkdir -p "$OUT"
SRC="$ROOT/crates/pedradb-core/src/bloom.rs"
echo "      charon=$CHARON"
(
  cd "$CRATE"
...
{
  echo "path=crates/pedradb-core/src/bloom.rs"
