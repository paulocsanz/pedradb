...
# Gate: min(3 rounds) > 1.0 em toda shape oficial, raftlog e blob inclusos.
# ROCKS_PARITY_BIG=0: sem seed 2^20 (leftover 1.1M no raftlog).
# rm do DB depois de cada processo. Isolated raftlog/blob: DB vazio.
...
# Isolated raftlog + blob: empty DB, named leftover cannot apply.
for r in 1 2 3; do
  ...
  echo "=== isolated r$r blob-compat $(date -u +%H:%M:%S) ==="
  run_compat "$OUT/iso$r/blob-pedra" kvrocks kvrocks_blob_set || echo "ISO-FAIL blob pedra r$r"
  echo "=== isolated r$r blob-rocks $(date -u +%H:%M:%S) ==="
  run_rocks "$OUT/iso$r/blob-rocks" kvrocks kvrocks_blob_set || echo "ISO-FAIL blob rocks r$r"
done

echo "--- P0.3 gate min>1.0 ---"
python3 - "$OUT" <<'PY'
...
print("P03_COLUNA_A rounds=3 peer=rocks-default(sync=false) async=PEDRA_PARITY_ASYNC=1 BIG=0")
fails = []
mins = []
for s in GATED:
...
# Isolated confirmation (not the official table).
print("P03_ISOLATED deps_raftlog / kvrocks_blob_set empty-DB")
for kind, shape in (("raft", "deps_raftlog"), ("blob", "kvrocks_blob_set")):
    rs = []
    for r in (1, 2, 3):
        p = qps(out / f"iso{r}/{kind}-pedra/rocks_parity_bench.json").get(shape)
        q = qps(out / f"iso{r}/{kind}-rocks/rocks_parity_bench.json").get(shape)
        if p and q and q > 0:
            rs.append(p / q)
            pb = lat(out / f"iso{r}/{kind}-pedra/rocks_parity_bench.json", shape)
            qb = lat(out / f"iso{r}/{kind}-rocks/rocks_parity_bench.json", shape)
            extra = ""
            if pb and qb:
                extra = (
            ...
            print(f"  ISO {shape} r{r} ratio={p/q:.3f}{extra}")
    if rs:
        print(f"  ISO {shape:26s} min={min(rs):.3f} median={statistics.median(rs):.3f}")

if fails:
    print(f"RESULT=P03_FAIL min_ratio={min(mins) if mins else 'nan'} fail={','.join(fails)}")
    sys.exit(1)
print(f"RESULT=P03_PASS min_ratio={min(mins):.3f}")
PY
rc=$?
echo "RESULT=P03_DONE rc=$rc $(date -u +%Y-%m-%dT%H:%M:%SZ)"
mkdir -p /data
echo "rc=$rc ts=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >/data/p03-last-result.txt 2>/dev/null || true
sleep infinity
