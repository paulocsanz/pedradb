#!/usr/bin/env bash
# Extract production isolated_kernel.rs via the standalone crate.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/isolated-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/isolated_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/isolated_kernel.llbc"
{
  echo "path=crates/pedradb-fold/src/isolated_kernel.rs"
