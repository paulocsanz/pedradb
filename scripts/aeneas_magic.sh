#!/usr/bin/env bash
# Extract production sst/magic_kernel.rs catalog entries (sst_magic_is_pedra).
# The shim re-exposes table.rs's SST_MAGIC const; the script fails if the
# copy drifts from the production line.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/magic-kernel"
OUT="$ROOT/formal/aeneas/out"
REQUIRED=0
if [[ "${1:-}" == "--required" ]]; then
  REQUIRED=1
fi
CHARON="${CHARON:-$(command -v charon || true)}"
AENEAS="${AENEAS:-$(command -v aeneas || true)}"
if [[ -z "$CHARON" || -z "$AENEAS" ]]; then
  msg="charon/aeneas not on PATH. See formal/aeneas/PINS.md"
  if [[ "$REQUIRED" -eq 1 ]]; then echo "FAIL  $msg" >&2; exit 1; fi
  echo "skip  $msg"; exit 0
fi
SRC="$ROOT/crates/pedradb-core/src/sst/magic_kernel.rs"
TABLE="$ROOT/crates/pedradb-core/src/sst/table.rs"
# Const pin: shim SST_MAGIC must equal the production table.rs line.
if ! grep -q 'pub const SST_MAGIC: &\[u8; 8\] = b"PEDRSST\\0";' "$TABLE"; then
  echo "FAIL  production table.rs SST_MAGIC line changed — update the shim const" >&2
  exit 1
fi
mkdir -p "$OUT"
echo "      charon=$CHARON"
( cd "$CRATE" && "$CHARON" cargo --preset=aeneas \
    --start-from 'crate::magic_kernel::sst_magic_is_pedra' \
    --start-from 'crate::magic_kernel::sst_magic_is_pedra_as_is' \
    --dest-file "$OUT/magic_kernel.llbc" )
set +e
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/magic_kernel.llbc"
ae=$?
set -e
if [[ $ae -ne 0 ]]; then
  echo "FAIL  aeneas backend lean exited $ae" >&2
  exit $ae
fi
if grep -q 'sorry' "$OUT/lean/MagicKernel.lean"; then
  echo "FAIL  MagicKernel.lean contains sorry" >&2
  exit 1
fi
{
  echo "path=crates/pedradb-core/src/sst/magic_kernel.rs"
  echo "sha256=$(shasum -a 256 "$SRC" | awk '{print $1}')"
  echo "aeneas=$("$AENEAS" -version 2>/dev/null | awk '{print $NF}')"
  echo "charon=$("$CHARON" version 2>/dev/null | head -1)"
} > "$OUT/SOURCE.magic"
echo "ok    extract magic → $OUT"
