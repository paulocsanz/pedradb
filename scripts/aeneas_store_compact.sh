#!/usr/bin/env bash
# Extract production crates/pedradb-store/src/compact_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/store-compact-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/store_compact_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/store_compact_kernel.llbc"
{
  echo "path=crates/pedradb-store/src/compact_kernel.rs"
...
} > "$OUT/SOURCE.store_compact"
echo "ok    extract store_compact → $OUT"
