#!/usr/bin/env bash
# Extract production crates/pedradb-store/src/snapshot_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/snapshot-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/snapshot_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/snapshot_kernel.llbc"
{
  echo "path=crates/pedradb-store/src/snapshot_kernel.rs"
