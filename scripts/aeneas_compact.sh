#!/usr/bin/env bash
# Extract production crates/pedradb-core/src/compact_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/compact-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/compact_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/compact_kernel.llbc"
{
  echo "path=crates/pedradb-core/src/compact_kernel.rs"
