#!/usr/bin/env bash
# Extract production crates/montanha-fdb-recipes/src/children_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/children-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/children_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/children_kernel.llbc"
{
  echo "path=crates/montanha-fdb-recipes/src/children_kernel.rs"
