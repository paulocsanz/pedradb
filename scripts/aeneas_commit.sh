#!/usr/bin/env bash
# Extract production commit_kernel.rs via the standalone crate.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/commit-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/commit_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/commit_kernel.llbc"
{
  echo "path=crates/pedradb-raft/src/commit_kernel.rs"
