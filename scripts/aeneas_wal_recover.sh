#!/usr/bin/env bash
# Extract production wal/recover_kernel.rs (+ wal/format.rs) via the shim crate.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/wal-recover-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/wal_recover_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/wal_recover_kernel.llbc"
{
  echo "path=crates/pedradb-core/src/wal/recover_kernel.rs"
...
} > "$OUT/SOURCE.wal_recover"
echo "ok    extract wal_recover → $OUT"
