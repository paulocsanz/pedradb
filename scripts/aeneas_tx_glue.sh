#!/usr/bin/env bash
# Extract production crates/pedradb-store/src/tx_glue_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/tx-glue-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/tx_glue_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/tx_glue_kernel.llbc"
{
  echo "path=crates/pedradb-store/src/tx_glue_kernel.rs"
