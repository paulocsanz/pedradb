#!/usr/bin/env bash
# Extract production crates/rocksdb-compat/src/iter_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/iter-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/iter_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/iter_kernel.llbc"
{
  echo "path=crates/rocksdb-compat/src/iter_kernel.rs"
