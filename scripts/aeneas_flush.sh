#!/usr/bin/env bash
# Extract production flush_kernel.rs (RFC-0174 P1.3).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/flush-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/flush_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/flush_kernel.llbc"
{
  echo "path=crates/pedradb-core/src/flush_kernel.rs"
