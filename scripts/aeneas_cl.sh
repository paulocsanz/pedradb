#!/usr/bin/env bash
# Extract production crates/pedradb-http/src/cl_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/cl-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/cl_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/cl_kernel.llbc"
{
  echo "path=crates/pedradb-http/src/cl_kernel.rs"
