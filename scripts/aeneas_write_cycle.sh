#!/usr/bin/env bash
# Extract production write_cycle_kernel.rs (RFC-0222 P0.7).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/write-cycle-kernel"
...
mkdir -p "$OUT"
SRC="$ROOT/crates/pedradb-core/src/write_cycle_kernel.rs"
echo "      charon=$CHARON"
( cd "$CRATE" && "$CHARON" cargo --preset=aeneas --dest-file "$OUT/write_cycle_kernel.llbc" )
set +e
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/write_cycle_kernel.llbc"
ae=$?
set -e
if [[ ! -f "$OUT/lean/WriteCycleKernel.lean" ]]; then
  echo "FAIL  WriteCycleKernel.lean missing (aeneas exit $ae)" >&2
  exit 1
fi
python3 - "$OUT/lean/WriteCycleKernel.lean" <<'PYEOF'
import sys
p = sys.argv[1]
...
PYEOF
{
  echo "path=crates/pedradb-core/src/write_cycle_kernel.rs"
...
} > "$OUT/SOURCE.write_cycle"
echo "ok    extract write_cycle → $OUT"
