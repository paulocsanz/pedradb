...
| **clones** | Raft vs store copies of `recover_commit` / `ae_ack_success` / `vote_decision` / `ae_entry_action` drifting, their AS-IS mutants (`*_as_is_*`) drifting, journal vs stream copies of `peek_pins_cursor` (F54) drifting, and a token-identical production fn duplicated across a pair but left unregistered (test-module fns exempt) |
