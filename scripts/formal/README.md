...
|-------|-----------------|
| **proof vs campaign** | RFC-0166 P2.4: every catalog pair is a **proof object** (machine-checked twin) or a **campaign gate** (`l28_*` prefix, named plant / seed replay — not ∀ traces). D1/R1/T1/C1 refinements (`d1_modelo` / `r1_modelo` / `t1_modelo` / `c1_modelo` and the spec ids) must stay proof. Negative: `test_proof_vs_campaign.py`. |
...
| **clones** | Raft vs store copies of `recover_commit` / `ae_ack_success` / `vote_decision` / `ae_entry_action` drifting, their AS-IS mutants (`*_as_is_*`) drifting, journal vs stream copies of `peek_pins_cursor` (F54) drifting, a token-identical production fn duplicated across a pair but left unregistered, and any identical production pair across the frozen kernels missing from the catalog entirely (test-module fns exempt) |
| **twins** | Missing `twin_kind`; a **close** twin without the entry `fn`; kernel decision tokens (`==`, `0xff`, `WouldGrant`, …) missing from the Verus twin. RFC-0166 P2.5: `test_twin_mutation.py` injects syntactic (rename entry) and semantic (empty body / missing tokens) drift in a cloned twin and requires FAIL naming the pair. |
