...
| **clones** | Raft vs store copies of `recover_commit` / `ae_ack_success` / `vote_decision` / `ae_entry_action` drifting, their AS-IS mutants (`*_as_is_*`) drifting, and journal vs stream copies of `peek_pins_cursor` (F54) drifting |
