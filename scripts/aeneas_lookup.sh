#!/usr/bin/env bash
# Extract production crates/pedradb-core/src/lookup_kernel.rs.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/lookup-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/lookup_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/lookup_kernel.llbc"
{
  echo "path=crates/pedradb-core/src/lookup_kernel.rs"
