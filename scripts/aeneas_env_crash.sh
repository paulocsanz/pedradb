#!/usr/bin/env bash
# Extract production env_crash_kernel.rs (+ group_commit_kernel.rs) via the shim crate.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CRATE="$ROOT/formal/aeneas/env-crash-kernel"
...
(
  cd "$CRATE"
  "$CHARON" cargo --preset=aeneas --dest-file "$OUT/env_crash_kernel.llbc"
)
"$AENEAS" -backend lean -dest "$OUT/lean" "$OUT/env_crash_kernel.llbc"
{
  echo "path=crates/pedradb-core/src/env_crash_kernel.rs"
...
} > "$OUT/SOURCE.env_crash"
echo "ok    extract env_crash → $OUT"
