...
ENROLLED: list[tuple[str, str, str]] = [
    ...
    ("BloomKernel.lean", "BloomFilter.may_contain_loop", "catalog:bloom_may_contain"),
]
...
#   one_step_per_remaining — one step per remaining unit; the bound IS
#   the steps_le twin (short-circuits only ever pay less).
...
TWIN_SHAPES: dict[str, dict[str, str]] = {
    ...
    "catalog:bloom_may_contain": {
        "shape": "one_step_per_remaining",
        "out": "BloomMayContainDerived.lean",
        "kernel_lib": "BloomKernel",
        "kernel_ns": "pedra_aeneas_bloom_kernel",
        "steps_def": "bloom_probe_steps",
        "steps_le_thm": "bloom_probe_steps_le",
        "bound_thm": "bloom_may_contain_work_bound",
        "bridges_file": "BloomBridges.lean",
        "anchor_defs": ["BloomFilter.may_contain_loop"],
    },
}
...
def _render_one_step_per_remaining(s: dict) -> str:
    """one_step_per_remaining template: the bound IS the steps_le twin
    — a short-circuit only ever pays less than the full pass."""
    return f"""-- AUTO-GENERATED by scripts/ratchet/derive_count_annotations.py
...
theorem {s['bound_thm']} : ∀ (k : Nat),
    {s['steps_def']} k ≤ k := by
  intro k
  exact {s['steps_le_thm']} k
"""
...
SHAPE_RENDERERS = {
    ...
    "one_step_per_remaining": _render_one_step_per_remaining,
}
