...
//! (`scripts/aeneas_lease.sh`). A Verus stand-in of `lease_live` billed as
...
mod tests {
    ...
    #[test]
    fn lease_kernel_has_no_verus_cartoon() {
        let src = include_str!("lease_kernel.rs");
        let block = concat!("verus", "!", " {");
        let cfg = concat!("cfg(", "verus", "_keep", "_ghost)");
        assert!(!src.contains(block), "stand-in is not last-wins of rustc lease_live");
