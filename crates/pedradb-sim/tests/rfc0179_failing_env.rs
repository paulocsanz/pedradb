...
/// RFC-0179: `rotate_wal_ignoring_pin` below hard is DiskPressure (no WAL
/// truncate); live key stays; not a durability fence.
#[test]
fn failing_env_rotate_wal_under_hard_floor_is_disk_pressure() {
    use pedradb_core::disk_pressure_kernel::compact_allowed_under_pressure;
    assert!(!compact_allowed_under_pressure(Some(1024)));
    let dir = tmp();
    let env = FailingEnv::passing();
    ...
    db.put(b"k", b"v").unwrap();
    handle.set_available_bytes(Some(1024));
    let err = db.rotate_wal_ignoring_pin().unwrap_err();
    assert!(
        matches!(
            err,
            CoreError::DiskPressure {
...
/// RFC-0179: `persist_manifest_durable` below hard is DiskPressure (no
/// MANIFEST rewrite); live key stays; not a durability fence.
#[test]
fn failing_env_persist_manifest_under_hard_floor_is_disk_pressure() {
    use pedradb_core::disk_pressure_kernel::compact_allowed_under_pressure;
    assert!(!compact_allowed_under_pressure(Some(1024)));
    let dir = tmp();
    let env = FailingEnv::passing();
    ...
    db.put(b"k", b"v").unwrap();
    handle.set_available_bytes(Some(1024));
    let err = db.persist_manifest_durable().unwrap_err();
    assert!(
        matches!(
            err,
            CoreError::DiskPressure {
...
/// RFC-0179: `compact_vlog_promote` below hard is DiskPressure (no `.new`
/// rename); live key stays; not a durability fence.
#[test]
fn failing_env_compact_vlog_promote_under_hard_floor_is_disk_pressure() {
    use pedradb_core::disk_pressure_kernel::compact_allowed_under_pressure;
    assert!(!compact_allowed_under_pressure(Some(1024)));
    let dir = tmp();
    let env = FailingEnv::passing();
    ...
    db.put(b"k", b"v").unwrap();
    handle.set_available_bytes(Some(1024));
    let err = db.compact_vlog_promote().unwrap_err();
    assert!(
        matches!(
            err,
            CoreError::DiskPressure {
