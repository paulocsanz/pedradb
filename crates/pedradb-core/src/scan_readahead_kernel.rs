...
mod tests {
    ...
    fn cap_never_splits_a_block() {
        ...
        let w = scan_readahead_window(&blocks, 0, true);
        assert_eq!(w.len, 200 * KIB, "two 100 KiB blocks fit, third would cross the cap");
    ...
    fn tail_and_oob_anchors_are_zero() {
        ...
        assert_eq!(scan_readahead_window(&blocks, 2, true), ScanReadaheadWindow::NONE);
        assert_eq!(scan_readahead_window(&blocks, 7, true), ScanReadaheadWindow::NONE);
    }
