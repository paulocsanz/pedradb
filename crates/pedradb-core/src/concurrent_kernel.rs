...
use crate::key::{InternalKey, SequenceNumber, MAX_SEQUENCE_NUMBER};
...
fn note_lsm_lock() {
    GET_LSM_LOCKS.fetch_add(1, Ordering::Relaxed);
}

/// RFC-0235 P1.1: LSM locks taken on the get path.
#[must_use]
pub fn get_lsm_lock_count() -> u64 {
    GET_LSM_LOCKS.load(Ordering::Relaxed)
}

/// RFC-0235 P1.1: zero the get-path LSM lock counter.
pub fn reset_get_lsm_lock_count() {
    GET_LSM_LOCKS.store(0, Ordering::Relaxed);
}

/// RFC-0236: gets served from the published SST snapshot (write lock held).
#[must_use]
pub fn get_used_published_sv_count() -> u64 {
    GET_USED_PUBLISHED_SV.load(Ordering::Relaxed)
}

/// RFC-0236: zero SuperVersion fallback counter (tests).
pub fn reset_get_used_published_sv_count() {
    GET_USED_PUBLISHED_SV.store(0, Ordering::Relaxed);
}
...
struct PendingWrite {
    ops: Vec<BatchOp>,
    do_sync: bool,
    /// Writers in flight at this member's `submit` entry (the `active`
    /// counter the enqueue saw). RFC-0211 follow-up: the seal policy's
    /// oversubscription signal — writers piled up *inside* submit — as
    /// opposed to the post-loop `active`, which a 50-client herd's
    /// client-side gaps can drop below `ncpu`.
    submit_active: usize,
...
struct WriteGroup {
    ...
    /// RFC-0211 P0.2 → RFC-0233 P1.4: the single-op async group
    /// boundary (`rmw_sched_kernel`: merge also when single-op
    /// writers ≥ 2, the writers == ncpu rmw regime). Default off
    /// since RFC-0233 P1.4 (the mmap WAL made the merge machinery a
    /// net loss — see the constructor); `PEDRA_RMW_SCHED=1` restores.
    /// Read once at open.
    rmw_sched: bool,
    /// RFC-0211 follow-up (2026-09-14) / RFC-0226 P0.2: seal async-only
    /// groups at the first drain when the leader is still alone in the
    /// batch (`seal_async_first_drain` — join/collect wait on the
    /// scheduler with no fd to amortize; latecomers still absorb into
    /// the open flight). `PEDRA_SEAL_ASYNC=0` restores the waits.
    seal_async: bool,
    /// RFC-0226 P1.1: opt-in — a still-alone first drain takes
    /// `commit_async_one` instead of the group serial section.
    /// Default off = AS-IS (stay in `lead()`). `PEDRA_SOLO_BYPASS=1`
    /// enables; flip of default is P1.2 (meter).
    solo_bypass: bool,
...
/// RFC-0229 P1.1: who owns this wait. Without the `pct` feature the
/// harness never binds a worker, so the token is always OsPark.
fn write_group_wait_token() -> crate::group_commit_kernel::WriteGroupWait {
    #[cfg(feature = "pct")]
    {
        crate::group_commit_kernel::write_group_wait_grant(crate::pct_hooks::harness_owns_wait())
    }
    #[cfg(not(feature = "pct"))]
    {
        crate::group_commit_kernel::write_group_wait_grant(false)
    }
}

fn granted_block<T>(site: &'static str, f: impl FnOnce() -> T) -> T {
    match write_group_wait_token() {
        crate::group_commit_kernel::WriteGroupWait::HarnessGrant => {
            #[cfg(feature = "pct")]
            {
                crate::pct_hooks::blocking_section(site, f)
            }
            #[cfg(not(feature = "pct"))]
            {
                let _ = site;
        ...
        crate::group_commit_kernel::WriteGroupWait::OsPark => f(),
    }
}

fn granted_sleep(site: &'static str, d: Duration) {
    granted_block(site, || std::thread::sleep(d));
}
...
impl WriteGroup {
    fn new() -> Self {
        Self {
            ...
            // RFC-0233 P1.4 (2026-09-16): default OFF. The RFC-0211
            // merge (default ON for overwrite_mc4) is obsolete under
            // the mmap WAL: a merged group saves ~0.4µs of memcpy but
            // pays queue + leader + channel reply (~5µs/op), and
            // staggered ycsb_f arrivals drained at avg_group 1.10.
            // Same-boot Darwin interleaved A/B (8 runs): overwrite_mc4
            // 114–208k merged vs 330–381k bypass; ycsb_f_mc4 min-of-3
            // 0.732→0.942. `PEDRA_RMW_SCHED=1` restores the merge.
            rmw_sched: match std::env::var("PEDRA_RMW_SCHED").as_deref() {
                Ok("1") | Ok("true") => true,
                _ => false,
            },
            // Default ON — the measured rank-1 hole is the async-only
            // leader waiting (cw=10.9µs/grp vs ~3.6µs/commit of engine
            // work on overwrite_mc4, same boot 2026-09-14).
            seal_async: match std::env::var("PEDRA_SEAL_ASYNC").as_deref() {
                Ok("0") | Ok("false") => false,
                _ => true,
            },
            // RFC-0233 P1.4: default ON. ycsb_f_mc4 is get+put; puts rarely
            // overlap so the rmw_sched merge was a group-of-1 (mpsc+Mutex)
            // instead of commit_async_one mmap. `PEDRA_SOLO_BYPASS=0` restores
            // the group serial section for a still-alone first drain.
            solo_bypass: match std::env::var("PEDRA_SOLO_BYPASS").as_deref() {
                Ok("0") | Ok("false") => false,
                _ => true,
            },
    ...
    fn commit_async_locked<E: Env>(
        ...
        let mut g = db.write();
        let result = if crate::write_admission_kernel::one_op_commit(ops.len() as u64) {
            g.commit_async_one(ops.pop().expect("len checked"))
        } else {
            g.commit_async_ops(ops)
        };
        if result.is_ok() {
    ...
    fn acquire_bypass_write<'a, E: Env>(
        &self,
        db: &'a RwLock<Db<E>>,
        fair: bool,
    ) -> parking_lot::RwLockWriteGuard<'a, Db<E>> {
    ...
    /// Concurrent async 1-op bypass (`overwrite_mc4`): memcpy+CRC of the
    /// Full record **off** `wal.lock()` (`encode_async_one_off_rwlock`).
    /// CS is ticket + optional &lt;HEADER pad. mmap (`job.run`) with the
    /// mutex free. Handles cloned under a brief `db.read()`. `begin_commit`
    /// via the inflight Arc (flush cannot rotate WAL). Apply into live
    /// `snapshot_mem` after WAL drop.
    fn commit_async_one_syscall_off_lock<E: Env>(
        &self,
        db: &RwLock<Db<E>>,
        batch: BatchOp,
        fair: bool,
        ...
        let _ = fair;
        let (wal, next_seq, class_w, inflight) = {
            let g = db.read();
            let inflight = g.commit_inflight_handle();
            // Fence before dropping the read lock so flush cannot take
            // `db.write()` + rotate WAL under a cloned `wal_arc`.
        ...
        let (op, seq, preframed) = match Db::<E>::encode_async_one_off_rwlock(
            &next_seq,
            &class_w,
            batch,
            self.phase_stats.as_ref(),
        ) {
            Ok(v) => v,
            Err(e) => {
        ...
        let t_lock = self.phase_stats.as_ref().map(|_| Instant::now());
        let mut w = wal.lock();
        if let (Some(st), Some(t_lock)) = (self.phase_stats.as_ref(), t_lock) {
            st.lock_wait_ns
                .fetch_add(t_lock.elapsed().as_nanos() as u64, Ordering::Relaxed);
        }
        let t1 = self.phase_stats.as_ref().map(|_| Instant::now());
        let job = match preframed {
            Some(frame) => match w.take_preframed_pwrite_job(frame) {
                Ok(Some(job)) => Ok(Some(job)),
                Ok(None) => {
                    match w.encode_write_op_batches(&[std::slice::from_ref(&op)]) {
                        Ok(_) => w.take_pwrite_job(),
                        Err(e) => Err(e),
                    }
                }
                Err(e) => Err(e),
            },
            None => match w.encode_write_op_batches(&[std::slice::from_ref(&op)]) {
                Ok(_) => w.take_pwrite_job(),
                Err(e) => Err(e),
            },
        };
        ...
        let io = match job {
            Err(e) => Err(e),
            Ok(Some(job)) => match job.run() {
                Ok((ticket, len)) => {
                    wal.lock().finish_pwrite(ticket, len);
                    Ok(())
                }
                Err(e) => {
                    wal.lock().finish_pwrite(0, 0);
                    Err(e)
                }
            },
            Ok(None) => wal.lock().write_pending_frame_lone(),
        };
        if let (Some(st), Some(t1)) = (self.phase_stats.as_ref(), t1) {
            st.wal_ns
        ...
        let g = db.read();
        let t2 = self.phase_stats.as_ref().map(|_| Instant::now());
        g.snapshot_mem().write().insert(
            InternalKey::new(op.key.clone(), op.sequence, op.kind),
            op.value.clone(),
        );
        if let (Some(st), Some(t2)) = (self.phase_stats.as_ref(), t2) {
            st.mem_ns
                .fetch_add(t2.elapsed().as_nanos() as u64, Ordering::Relaxed);
        }
        g.note_dirty_points(std::slice::from_ref(&op));
        let t3 = self.phase_stats.as_ref().map(|_| Instant::now());
        g.publish_sequence(seq);
        if let (Some(st), Some(t3)) = (self.phase_stats.as_ref(), t3) {
            st.publish_ns
        ...
        if let Some(mut gw) = db.try_write() {
            ...
            gw.maybe_auto_flush_best_effort();
            if let (Some(st), Some(t4)) = (self.phase_stats.as_ref(), t4) {
                st.flush_check_ns
    ...
    fn await_flush_debt<E: Env>(&self, db: &RwLock<Db<E>>) {
        ...
        loop {
            ...
            granted_sleep("flush_debt", FLUSH_DEBT_POLL);
    ...
    fn await_l0_park<E: Env>(&self, db: &RwLock<Db<E>>) {
        ...
        loop {
            ...
            granted_sleep("l0_stall", FLUSH_DEBT_POLL);
            waited += FLUSH_DEBT_POLL;
        }
    }
    ...
    fn submit_after_begin<E: Env>(
        ...
        // RFC-0211 → RFC-0233 P1.4: `rmw_sched` is default OFF (the
        // mmap WAL made the merged single-op group a net loss);
        // `PEDRA_RMW_SCHED=1` restores the merge at writers == ncpu
        // (overwrite_mc4 / ycsb_f_mc4).
        // Multi-op (apply/raftlog) stays on the 0201 bypass.
        ...
        if occ.is_none() && active == 1 && !self.recently_concurrent() {
            ...
            let result = match crate::changelog_kernel::changelog_durable_commit_fate(
                ...
                crate::changelog_kernel::ChangelogCommitFate::Skip => {
                    self.commit_async_locked(db, ops)
                }
            };
            self.finish_lone();
            return result;
            ...
            && !async_merged
        {
            ...
            // with a direct handoff to the next waiter.
            let fair = self.write_fair;
            let n = ops.len() as u64;
            let result = if ops.len() == 1 {
                self.commit_async_one_syscall_off_lock(
                    db,
                ...
                let mut guard = self.acquire_bypass_write(db, fair);
        ...
        let (reply, rx) = {
            ...
            if leader {
                ...
                // G1/sync: yield so waiters push before `lead()` re-locks
                // (Rocks LinkOne is wait-free; we only have a mutex).
                // RFC-0233 P1.4: `seal_async` seals the first drain at
                // batch=1, so this yield never grew groups (ycsb_f_mc4
                // avg_group=1.37) and donated ~5–10 µs per group-of-1.
                // Waiters that arrive during encode still join via the
                // absorb loop in `finish_group_off_lock`.
                if !self.seal_async {
    ...
    fn lead<E: Env>(&self, db: &RwLock<Db<E>>) -> Result<SequenceNumber> {
        ...
        loop {
            ...
            loop {
                ...
                // stop the join yield-loop at the first drain (the loop
                // waits on the scheduler; peers are in client gaps). The
                // bound skip below re-derives the same policy so every
                // exit from this loop — join_complete included — seals.
                // `submit_active` max = writers piled inside submit (the
                // oversubscription signal), not the gap-exposed `active`.
                if self.seal_async
                    && !batch.is_empty()
                    && crate::group_window_kernel::seal_async_first_drain(
                        batch.iter().map(|p| p.submit_active).max().unwrap_or(0),
                        self.axis_ncpu,
                        batch.iter().any(|p| {
            ...
            // RFC-0226 P1.1: still-alone first drain (empty queue, no
            // in-submit peer) takes commit_async_one instead of the
            // group serial section. Async-only — a sync member must
            // keep the group fd path (OCC WriteOptions::sync, G1).
            let any_sync = batch
                .iter()
                .any(|p| crate::write_admission_kernel::wal_sync_required(true, p.do_sync, false));
            if crate::group_window_kernel::solo_bypass_armed(self.solo_bypass) && !any_sync {
                let queue_len = self.queue.lock().pending.len();
                let active = self.active.load(Ordering::Relaxed);
                if crate::group_window_kernel::solo_leader_bypass(batch.len(), queue_len, active) {
                    let mut pending = batch.pop().expect("solo_leader_bypass pins batch_len==1");
                    let n = pending.ops.len() as u64;
                    let mut guard = db.write();
                    Self::validate_occ_batch(&mut guard, std::slice::from_mut(&mut pending));
                    let result = if let Some(e) = pending.occ_err.take() {
                        drop(guard);
                        Err(e)
                    } else if crate::write_admission_kernel::one_op_commit(pending.ops.len() as u64)
                        && pending.occ.is_none()
                    {
                        drop(guard);
                        self.commit_async_one_syscall_off_lock(
                            db,
                            pending.ops.pop().expect("len checked"),
                            false,
                        )
                    } else if crate::write_admission_kernel::one_op_commit(pending.ops.len() as u64)
                    {
                        let result =
                            guard.commit_async_one(pending.ops.pop().expect("len checked"));
                        if result.is_ok() {
                            self.publish_apply(&guard);
                        }
                        drop(guard);
                        ...
                        let result = guard.commit_async_ops(std::mem::take(&mut pending.ops));
                        if result.is_ok() {
                            self.publish_apply(&guard);
                        }
                        drop(guard);
                    ...
                    self.batches.fetch_add(1, Ordering::Relaxed);
                    self.batch_ops.fetch_add(n, Ordering::Relaxed);
                    self.last_group_len.store(1, Ordering::Relaxed);
                    match pending.reply {
            ...
            // one `write()` per group) is the whole win. `any_sync` was
            // derived above (solo_bypass must not steal a G1 member).
            // RFC-0211 follow-up: the seal policy derived once, post-loop,
            // from the sealed batch itself — whichever way the join loop
            // exited. Async-only, writers ≤ ncpu ⇒ no waits below. The
            // writers signal is the members' submit-time `active` max
            // (writers inside submit), so an oversubscribed herd keeps
            // its collect even when the live counter dips in gaps.
            let sealed_async = self.seal_async
                && !batch.is_empty()
                && crate::group_window_kernel::seal_async_first_drain(
                    batch.iter().map(|p| p.submit_active).max().unwrap_or(0),
                    self.axis_ncpu,
            ...
            let bound = if sealed_async {
                // RFC-0211 follow-up: this async-only group seals — every
                // collect below waits on the scheduler with no fd to
                // amortize (measured: cw=10.9µs/grp vs ~3.6µs/commit
                // engine work on overwrite_mc4). Latecomers still join
                // via the absorb loop after `group_start`.
                None
            } else if batch_ops < CATCHUP_SKIP_OPS {
                ...
                if collect_mode {
                    ...
                    if herd_only {
                        ...
                        loop {
                            ...
                            let woke = granted_block("group_collect", || {
                                self.arrived.wait_for(&mut g, slice)
                            });
                            batch.extend(g.pending.drain(..));
                    ...
                    while batch.len() < self.active.load(Ordering::Relaxed) {
                        ...
                        let _timed_out = granted_block("group_collect", || {
                            self.arrived.wait_for(&mut g, deadline - now)
                        });
                        batch.extend(g.pending.drain(..));
                    }
                }
            ...
            self.last_group_len.store(batch.len(), Ordering::Relaxed);
    ...
    fn finish_group_off_lock<E: Env>(
        ...
        let io_err = {
            ...
            // `take_pwrite_job` Err is WAL I/O — never `.ok().flatten()` into
            // the empty-frame write_pending path (that would publish a lie).
            // Bind the Result so the parking_lot MutexGuard drops before the
            // match (Rust 2021 keeps a match-scrutinee temporary alive for
            // the whole match; a nested `wal.lock()` would same-thread deadlock).
            let job = wal.lock().take_pwrite_job();
            let e = match job {
                Err(e) => Some(e),
                Ok(Some(job)) => match job.run() {
                    Ok((ticket, len)) => {
            ...
            if pinned {
                ledger_bytes = wal.lock().position().saturating_sub(before);
            }
            e
        };
...
pub struct ConcurrentDb<E: Env = StdEnv> {
    ...
    /// RFC-0236 SuperVersion: LSM snapshot readers clone without the Db write lock.
    published_ssts: Arc<RwLock<Arc<PublishedSv>>>,
    /// Endure point-miss / point-hit (RFC-0235); same atomics as `Db`.
    class_z0: Arc<AtomicU64>,
    class_z1: Arc<AtomicU64>,
...
include!("concurrent_put_kernel.rs");
...
impl<E: Env> ConcurrentDb<E> {
    ...
    pub fn from_db(db: Db<E>) -> Self {
        ...
        let class_z0 = db.class_z0_handle();
        let class_z1 = db.class_z1_handle();
    ...
    /// the snapshot has decided: found, deleted, or absent (no wait).
    ...
    fn scan_published(
        ...
        let mut out = Vec::new();
        {
            ...
            for (k, v) in mem.iter_internal_iter_at(start, end, snapshot) {
        ...
        for table in sv.ssts.iter() {
            let iter = table.iter_user_range(
                ...
                snapshot,
                true,
                Box::new(|bi| table.decode_block(bi).ok().map(Arc::new)),
            );
            for (k, v) in iter {
                if seen.contains(&k.user_key) {
                    continue;
                }
                seen.insert(k.user_key.clone());
                if k.kind == crate::key::ValueType::Value {
    ...
    /// RFC-0236: hold the Db write lock while `f` runs (compact-vs-get test).
    ///
    /// Publishes the SuperVersion first, matching `compact_l0_off_lock`
    /// prepare so readers clone the snapshot current at lock acquire.
    pub fn with_inner_write_lock<R>(&self, f: impl FnOnce() -> R) -> R {
    ...
    /// (point-cache or compat LAST_GET). Same atomics as [`Db::get`].
    pub fn note_class_point(&self, hit: bool) {
        if hit {
            self.class_z1.fetch_add(1, Ordering::Relaxed);
        } else {
            self.class_z0.fetch_add(1, Ordering::Relaxed);
        }
    }

    /// RFC-0235 Endure class of the current get/scan/put window.
    #[must_use]
    pub fn workload_class_now(&self) -> crate::WorkloadClass {
        self.inner.read().workload_class_now()
    }

    /// L0+L1 filter pins (Fjall per-level pin).
    #[must_use]
    pub fn hot_filter_pin_count(&self) -> u64 {
        self.inner.read().hot_filter_pin_count()
    }
        ...
        let job = {
            ...
            let prepared =
                if crate::write_admission_kernel::batch_is_empty(g.level_file_count(0) as u64) {
                    // RFC-0234 P1.3: L0 already drained — unstack L1.
                    g.prepare_stacked_l1_repair(opts)?
                } else {
                    g.prepare_l0_compact(opts)?
                };
            match prepared {
    ...
    /// RFC-0234 P1.3: one drain job from `src_level` into `src_level+1`
    /// (ignore size target). Settle uses this so a two-round MVCC count
    /// overlaps 1 SST. Not on the ingest worker.
    ///
    /// # Errors
    /// SST / MANIFEST I/O.
    pub fn compact_level_drain_off_lock(&self, src_level: u32) -> Result<bool> {
        let job = {
            let mut g = self.inner.write();
            let opts = CompactOptions {
                max_input_files: Some(2),
            ...
            self.publish_from(&g);
            match g.prepare_level_drain(src_level, opts)? {
                None => return Ok(false),
                Some(j) => j,
            }
        };
    ...
    /// [`Self::compact_level_drain_off_lock`] from L1.
    pub fn compact_l1_drain_off_lock(&self) -> Result<bool> {
        self.compact_level_drain_off_lock(1)
    }

    /// RFC-0234 P1.3: one stacked-level repair job at `level` (L2 after
    /// two-round sequential seed). Settle only — not the ingest worker.
    ///
    /// # Errors
    /// SST / MANIFEST I/O.
    pub fn compact_stacked_level_off_lock(&self, level: u32) -> Result<bool> {
        let job = {
            let mut g = self.inner.write();
            let opts = CompactOptions {
                max_input_files: Some(2),
            ...
            self.publish_from(&g);
            match g.prepare_stacked_level_repair(level, opts)? {
                None => return Ok(false),
                Some(j) => j,
            }
        };
    ...
    /// RFC-0234 P1.3: one adjacent-file coalesce job at `level` (under
    /// the compact split target). Settle only — not the ingest worker.
    ///
    /// # Errors
    /// SST / MANIFEST I/O.
    pub fn compact_adjacent_coalesce_off_lock(&self, level: u32) -> Result<bool> {
        let job = {
            let mut g = self.inner.write();
            let opts = CompactOptions {
                max_input_files: Some(64),
                ..CompactOptions::default()
            };
            self.publish_from(&g);
            match g.prepare_adjacent_coalesce(level, opts)? {
                None => return Ok(false),
                Some(j) => j,
            }
        };
    ...
    pub fn drain_l0_below_trigger(&self) -> Result<usize> {
        ...
        loop {
            ...
            if !crate::flush_kernel::l0_compact_due(
                l0 as u64,
                crate::db::L0_COMPACTION_TRIGGER as u64,
            ) {
                return Ok(n);
            }
    ...
    /// RFC-0234 P0.1: one L0→L1 job when live L0 is at/above the trigger.
    ...
    /// Bounded range at latest. Clones the published SuperVersion when the
    /// compact write lock is held (RFC-0236 P0.2).
    #[must_use]
    pub fn range_limited(
        ...
        if let Some(g) = self.inner.try_read() {
            return g.range_limited(start, end, limit);
        }
        GET_USED_PUBLISHED_SV.fetch_add(1, Ordering::Relaxed);
        let mut out = self.scan_published(start, end, MAX_SEQUENCE_NUMBER);
        if let Some(n) = limit {
            out.truncate(n);
        }
        out
    ...
    /// RFC-0236: clones the published SuperVersion when compact holds the write lock.
    #[must_use]
    pub fn scan_collect(&self, start: Bound<&[u8]>, end: Bound<&[u8]>) -> Vec<(Bytes, Bytes)> {
        ...
        GET_USED_PUBLISHED_SV.fetch_add(1, Ordering::Relaxed);
        self.scan_published(start, end, MAX_SEQUENCE_NUMBER)
    }
    ...
    pub fn scan_collect_at(
        ...
        self.reads_served.fetch_add(1, Ordering::Relaxed);
        if let Some(g) = self.inner.try_read() {
            return g.range_at_limited(snapshot, start, end, limit);
        }
        GET_USED_PUBLISHED_SV.fetch_add(1, Ordering::Relaxed);
        let mut out = self.scan_published(start, end, snapshot);
        if let Some(n) = limit {
            out.truncate(n);
        }
        Ok(out)
    ...
    pub fn recover_from_fence(&self) -> Result<Option<crate::db::FenceRecovery>> {
        ...
        while !crate::write_admission_kernel::batch_is_empty(self.commit_inflight() as u64) {
            ...
            granted_sleep("fence_drain", Duration::from_micros(200));
        }
    ...
    pub fn materialize_parked_once(&self) -> bool {
        ...
        if did {
            // RFC-0234 P0.1: 1 compact per L0 install, off the flush lock
            // so the worker can keep draining parked tables.
            self.maybe_compact_l0_at_trigger();
        }
        did
    }
    ...
    pub fn materialize_parked_once_try(&self) -> bool {
        ...
        let did = self.materialize_parked_holding_flush();
        drop(_flush);
        if did {
            self.maybe_compact_l0_at_trigger();
        }
...
mod tests {
    ...
    /// SuperVersion and do not wait. A one-SST hit-only check cannot catch a
    /// miss/scan that still takes `inner.read()`, nor oldest-first L0 order.
    #[test]
    fn rfc0236_get_does_not_wait_compact_write_lock() {
        let dir = temp_dir();
        ...
        assert!(
            stats.l0_files >= 2,
            "need two overlapping L0s, got l0={} sst={}",
            stats.l0_files,
            stats.sst_count
        );
        db.point_cache.clear();
        reset_get_used_published_sv_count();
        ...
        rx.recv().expect("write lock holder started");
        let bar = std::time::Duration::from_millis(150);

        let t0 = std::time::Instant::now();
        let hit = db.get(b"a");
        let hit_elapsed = t0.elapsed();
        assert_eq!(hit.as_deref(), Some(b"v2".as_ref()), "newest L0 must win");
        assert!(
            hit_elapsed < bar,
            "hit get waited on compact write lock for {hit_elapsed:?}"
        );

        let t1 = std::time::Instant::now();
        let miss = db.get(b"b");
        let miss_elapsed = t1.elapsed();
        assert_eq!(miss, None, "in-range absent key must be a decided miss");
        assert!(
            miss_elapsed < bar,
            "absent get waited on compact write lock for {miss_elapsed:?}"
        );

        let t2 = std::time::Instant::now();
        let scanned = db.scan_collect(Bound::Unbounded, Bound::Unbounded);
        let scan_elapsed = t2.elapsed();
        assert!(
            scan_elapsed < bar,
            "scan_collect waited on compact write lock for {scan_elapsed:?}"
        );
        assert_eq!(
            scanned
                .iter()
                ...
                .collect::<Vec<_>>(),
            vec![(b"a".as_ref(), b"v2".as_ref()), (b"c".as_ref(), b"vc".as_ref())]
        );
        assert!(
            get_used_published_sv_count() >= 1,
            "get/scan must use published SuperVersion while write lock is held"
        );
        holder.join().unwrap();
        drop(db);
        let _ = fs::remove_dir_all(&dir);
    ...
    /// RFC-0237 P0.1: the group leader drops the Db write lock before WAL I/O.
    /// Drives two concurrent G1 puts on the shipped `finish_group_off_lock`
    /// path (not a reimplementation).
    #[test]
    fn rfc0237_wal_leader_cs_drops_lock_before_io() {
        let src = include_str!("concurrent_kernel.rs");
        let off = src
            .split("fn finish_group_off_lock")
            .nth(1)
            .expect("finish_group_off_lock");
        let drop_at = off.find("drop(guard)").expect("drop(guard)");
        let io_at = off
            .find("write_pending_frame")
            .expect("write_pending_frame");
        assert!(
            drop_at < io_at,
            "group leader must drop the Db write lock before write_pending_frame"
        );
        assert!(
            !crate::group_commit_kernel::rwlock_client_may_mutate(false),
            "off-lock WAL I/O must not mutate Db"
        );

        let dir = temp_dir();
        let saw_free = Arc::new(AtomicBool::new(false));
        let wal_io = Arc::new(AtomicU64::new(0));
        let slot: Arc<Mutex<Option<ConcurrentDb<FenceEnv>>>> = Arc::new(Mutex::new(None));
        let mut env = FenceEnv::new();
        {
            let saw_free = Arc::clone(&saw_free);
            let wal_io = Arc::clone(&wal_io);
            let slot = Arc::clone(&slot);
            env.wal_io_probe = Arc::new(move || {
                wal_io.fetch_add(1, Ordering::Relaxed);
                if let Some(db) = slot.lock().as_ref() {
                    // Write lock free (peers may hold db.read() in apply).
                    if db.inner.try_read().is_some() {
                        saw_free.store(true, Ordering::Relaxed);
                    }
                }
            });
        }
        let db = ConcurrentDb::open_with_env(&dir, OpenOptions::default(), env.clone()).unwrap();
        *slot.lock() = Some(db.clone());
        // Same shape as `multi_writer_failed_sync_does_not_publish_group`:
        // catch-up window + barrier so G1 puts join `finish_group_off_lock`
        // instead of the lone path (which holds the write lock through fd).
        db.set_write_group_catchup_window(Duration::from_millis(20));
        db.put(b"warm", b"1").unwrap();
        saw_free.store(false, Ordering::Relaxed);
        wal_io.store(0, Ordering::Relaxed);
        let n = 4usize;
        ...
        assert!(
            wal_io.load(Ordering::Relaxed) > 0,
            "WAL I/O must run on the shipped group path"
        );
        assert!(
            saw_free.load(Ordering::Relaxed),
            "group leader WAL I/O must run with the Db write lock free"
        );
        for i in 0..n {
            let k = [b'k', u8::try_from(i).expect("n fits u8")];
    ...
    /// RFC-0237: concurrent async bypass (the overwrite_mc4 path) encodes
    /// the Full record **off** `wal.lock()`, tickets under the mutex, then
    /// mmap with both the Db write lock and the WAL mutex free.
    #[test]
    fn rfc0237_async_bypass_drops_lock_before_wal_io() {
        let src = include_str!("concurrent_kernel.rs");
        let off = src
            .split("fn commit_async_one_syscall_off_lock")
            .nth(1)
            .expect("commit_async_one_syscall_off_lock");
        let body = off.split("\n    fn ").next().expect("fn body");
        assert!(
            !body.contains("acquire_bypass_write"),
            "bypass encode must not take Db write lock"
        );
        assert!(
            body.contains("encode_async_one_off_rwlock"),
            "bypass encode is off the Db RwLock"
        );
        assert!(
            body.contains("class_w_handle()") && body.contains("&class_w"),
            "bypass must increment Endure W (class_w) on the off-rwlock encode"
        );
        assert!(
            !body.contains("drop(wal.lock())"),
            "try_lock fail must not convoy by taking wal.lock() just to drop it"
        );
        let grab = body
            .split("let (wal, next_seq, class_w, inflight)")
            .nth(1)
            .expect("wal/next_seq/class_w/inflight bind");
        let grab = grab
            .split("encode_async_one_off_rwlock")
            .next()
            .expect("encode follows bind");
        let read_at = grab.find("db.read()").expect("db.read() in bind");
        let add_at = grab.find("fetch_add(1").expect("fetch_add in bind");
        let scope_end = grab.rfind('}').expect("end of db.read() scope");
        assert!(
            read_at < add_at && add_at < scope_end,
            "inflight.fetch_add must precede the end of the db.read() scope"
        );
        assert!(
            add_at < grab.find("wal_arc()").unwrap_or(usize::MAX),
            "fence before cloning wal_arc so rotate cannot swap under the clone"
        );
        let inflight_at = body
            .find("fetch_add(1")
            .expect("begin_commit via inflight Arc");
        let encode_at = body
            .find("encode_async_one_off_rwlock")
            .expect("encode_async_one_off_rwlock");
        let lock_at = body.find("wal.lock()").expect("wal.lock ticket CS");
        assert!(
            inflight_at < encode_at && encode_at < lock_at,
            "memcpy/CRC (encode_async_one_off_rwlock) must run before wal.lock()"
        );
        let take_at = body
            .find("take_preframed_pwrite_job")
            .expect("take_preframed_pwrite_job");
        let drop_w_at = body[take_at..]
            .find("drop(w)")
            .map(|i| take_at + i)
            .expect("drop(w) after take_preframed_pwrite_job");
        let run_at = body.find("job.run()").expect("job.run");
        assert!(
            inflight_at < take_at && take_at < drop_w_at && drop_w_at < run_at,
            "bypass: begin_commit, ticket preframed, drop wal.lock(), then job.run"
        );
        let live_at = body.find("snapshot_mem()").expect("live snapshot_mem");
        let apply_end = body.find("g.end_commit()").expect("g.end_commit");
        assert!(
            drop_w_at < live_at && live_at < apply_end,
            "live snapshot_mem after WAL drop; g.end_commit"
        );

        let dir = temp_dir();
        let saw_free = Arc::new(AtomicBool::new(false));
        let wal_free = Arc::new(AtomicBool::new(false));
        let wal_io = Arc::new(AtomicU64::new(0));
        let slot: Arc<Mutex<Option<ConcurrentDb<FenceEnv>>>> = Arc::new(Mutex::new(None));
        let mut env = FenceEnv::new();
        {
            let saw_free = Arc::clone(&saw_free);
            let wal_free = Arc::clone(&wal_free);
            let wal_io = Arc::clone(&wal_io);
            let slot = Arc::clone(&slot);
            env.wal_io_probe = Arc::new(move || {
                let n = wal_io.fetch_add(1, Ordering::Relaxed);
                // First WAL I/O only: hold so the other three enter submit
                // (`write()` may fire this probe per chunk).
                if n == 0 {
                    std::thread::sleep(std::time::Duration::from_millis(8));
                }
                if let Some(db) = slot.lock().as_ref() {
                    // Write lock free (a peer may hold db.read() in apply).
                    if let Some(g) = db.inner.try_read() {
                        saw_free.store(true, Ordering::Relaxed);
                        if g.wal_arc().try_lock().is_some() {
                            wal_free.store(true, Ordering::Relaxed);
                        }
                    }
                }
            });
        ...
        *slot.lock() = Some(db.clone());
        ...
        for i in 0..n {
            ...
            handles.push(std::thread::spawn(move || {
                barrier.wait();
                for round in 0..8u8 {
                    let k = [b'k', u8::try_from(i).expect("n fits u8"), round];
                    db.put(&k, b"v").unwrap();
                }
            }));
        }
        ...
        assert!(
            wal_io.load(Ordering::Relaxed) > 0,
            "WAL I/O must run on the shipped async bypass"
        );
        assert!(
            saw_free.load(Ordering::Relaxed),
            "async bypass WAL I/O must run with the Db write lock free"
        );
        assert!(
            wal_free.load(Ordering::Relaxed),
            "async bypass mmap/pwrite must run with wal.lock() free (ticket under lock, I/O off it)"
        );
        for i in 0..n {
            for round in 0..8u8 {
                let k = [b'k', u8::try_from(i).expect("n fits u8"), round];
    ...
    /// RFC-0237: flush during off-lock WAL I/O must not orphan the apply.
    /// `wal_io_probe` calls `flush()` only when the Db write lock is free
    /// (bypass; lone holds it through I/O). After Ok, live get and
    /// close+reopen must see the key (`begin_commit` + insert into
    /// post-flush live mem).
    #[test]
    fn rfc0237_flush_during_wal_io_does_not_lose_put() {
        let dir = temp_dir();
        ...
        let mut env = FenceEnv::new();
        {
            ...
            env.wal_io_probe = Arc::new(move || {
                // FenceEnv probes every file write (WAL and SST). One flush
                // only — re-entering flush from L0 write deadlocks.
                if flushed
                ...
                if let Some(db) = slot.lock().as_ref() {
                    // Bypass I/O holds no Db lock. `try_write` loses to a
                    // peer's `db.read()` apply; `try_read` only fails if
                    // someone holds exclusive write (lone path). Then
                    // `flush()` waits for readers.
                    if db.inner.try_read().is_none() {
        ...
        *slot.lock() = Some(db.clone());
        ...
        assert!(
            ...
            "flush must run from wal_io_probe on the off-lock bypass"
        );
        for i in 0..n {
            let k = [b'k', u8::try_from(i).expect("n fits u8")];
            assert_eq!(
                db.get(&k).as_deref(),
                Some(&b"v"[..]),
                "live get must see {k:?} after flush-during-WAL"
            );
        }
        drop(slot.lock().take());
        db.close().unwrap();
        let reopened = ConcurrentDb::open(&dir).unwrap();
        for i in 0..n {
            let k = [b'k', u8::try_from(i).expect("n fits u8")];
            assert_eq!(
                reopened.get(&k).as_deref(),
                Some(&b"v"[..]),
                "close+reopen must see {k:?}"
            );
        }
    ...
    /// RFC-0237: 4-writer async bypass must increment Endure \(W\)
    /// (`class_w`) so WriteBurst L0-at-trigger is live. Pre-fix
    /// `encode_async_one_off_rwlock` skipped `class_w` and left \(W=0\).
    #[test]
    fn rfc0237_async_bypass_increments_class_w_write_burst() {
        let dir = temp_dir();
        ...
        assert_eq!(
            db.workload_class_now(),
            crate::WorkloadClass::WriteBurst,
            "4 async bypass puts must set W unique-max (WriteBurst)"
        );
        drop(db);
        let _ = fs::remove_dir_all(&dir);
    }

    /// RFC-0237: two concurrent async overwrites of the same key. After both
    /// Ok, live get equals the higher-seq value and matches close+reopen
    /// (WAL-held apply; dropping WAL before apply let the older seq win
    /// `tail_best`).
    #[test]
    fn rfc0237_concurrent_async_same_key_keeps_higher_seq() {
        let dir = temp_dir();
        let db = ConcurrentDb::open(&dir).unwrap();
        db.set_default_write_sync(false);
        let n = 2usize;
        ...
        for i in 0..n {
            ...
            handles.push(thread::spawn(move || {
                barrier.wait();
                let v = [b'a' + u8::try_from(i).expect("n fits u8")];
                let seq = db
                    .put_with_seq(b"k", &v, WriteOptions::no_sync())
                    .expect("async put");
                (seq, v)
            }));
        }
        let mut got: Vec<(SequenceNumber, [u8; 1])> = handles
            .into_iter()
            .map(|h| h.join().expect("join"))
            .collect();
        got.sort_by_key(|(seq, _)| *seq);
        assert_ne!(got[0].0, got[1].0, "two commits must mint distinct seqs");
        let (hi_seq, hi_val) = got[1];
        let live = db.get(b"k").expect("live get");
        assert_eq!(
            live.as_ref(),
            hi_val.as_ref(),
            "live get must be the higher seq ({hi_seq}), not the later insert"
        );
        db.close().unwrap();
        let reopened = ConcurrentDb::open(&dir).unwrap();
        let recovered = reopened.get(b"k").expect("reopen get");
        assert_eq!(
            recovered.as_ref(),
            live.as_ref(),
            "close+reopen must match the live higher-seq value"
        );
        reopened.close().unwrap();
        let _ = fs::remove_dir_all(&dir);
    }

    /// RFC-0237: async 1-op bypass vs `apply_batch` (len>1, same
    /// `!async_merged` branch). Holding WAL then `db.read()` deadlocks
    /// against the batch path (`db.write()` then `wal.lock()`). Both must
    /// Ok; a hang is the ABBA bug.
    #[test]
    fn rfc0237_async_one_op_and_batch_do_not_deadlock() {
        let dir = temp_dir();
        let wal_io = Arc::new(AtomicU64::new(0));
        let mut env = FenceEnv::new();
        {
            let wal_io = Arc::clone(&wal_io);
            env.wal_io_probe = Arc::new(move || {
                if wal_io.fetch_add(1, Ordering::Relaxed) == 0 {
        ...
        let one = {
            ...
            thread::spawn(move || {
                barrier.wait();
                let r = db.put_with(b"one", b"1", WriteOptions::no_sync());
                let _ = tx1.send(r);
            })
        };
        let batch = {
            ...
            thread::spawn(move || {
                ...
                let r = db.apply_batch_vec_with(
                    vec![BatchOp::put(b"b0", b"x"), BatchOp::put(b"b1", b"y")],
                    WriteOptions::no_sync(),
                );
                let _ = tx2.send(r.map(|_| ()));
            })
        };
        let r1 = rx1
            .recv_timeout(Duration::from_secs(10))
            .expect("1-op hung (WAL then db.read vs batch db.write then wal.lock)");
        let r2 = rx2
            .recv_timeout(Duration::from_secs(10))
            .expect("batch hung");
        r1.expect("1-op put");
        r2.expect("apply_batch");
        one.join().unwrap();
        batch.join().unwrap();
        assert_eq!(db.get(b"one").as_deref(), Some(&b"1"[..]));
        assert_eq!(db.get(b"b0").as_deref(), Some(&b"x"[..]));
        assert_eq!(db.get(b"b1").as_deref(), Some(&b"y"[..]));
        drop(db);
        let _ = fs::remove_dir_all(&dir);
    }

    /// RFC-0236/0237: a committed put still in mem is visible to get while
    /// another thread holds the write lock **without** `publish_from`.
    /// `with_inner_write_lock` publishes first and cannot catch a missing
    /// apply-time SuperVersion publish.
    #[test]
    fn rfc0237_get_sees_unflushed_put_under_write_lock() {
        let dir = temp_dir();
        let db = Arc::new(ConcurrentDb::open(&dir).unwrap());
        ...
        assert_eq!(
            got.as_deref(),
            Some(b"v".as_ref()),
            "unflushed put must be visible on the published SuperVersion"
        );
        assert!(
            elapsed < std::time::Duration::from_millis(150),
            "get waited on write lock for {elapsed:?}"
        );
        assert!(
            get_used_published_sv_count() >= 1,
            "get must use published SuperVersion while write lock is held"
        );
        holder.join().unwrap();
        drop(db);
        let _ = fs::remove_dir_all(&dir);
    ...
    /// RFC-0237: publish_apply must Arc-swap the live memtable, not
    /// deep-clone BTree/Bloom on every put (overwrite_mc4 DIAG collapse).
    #[test]
    fn rfc0237_publish_apply_is_arc_swap_not_mem_clone() {
        let dir = temp_dir();
        let db = ConcurrentDb::open(&dir).unwrap();
        for i in 0..512u32 {
            let k = format!("k{i:04}");
        ...
        let g = db.inner.read();
        let sv = db.published_ssts.read().clone();
        assert!(
            Arc::ptr_eq(&sv.mem, &g.snapshot_mem()),
            "SuperVersion mem must share the live mem Arc after apply"
        );
        drop(g);
    ...
    fn restore_wal_pwrite_env(
        prev_pw: Option<String>,
        prev_buf: Option<String>,
        prev_ag: Option<String>,
    ) {
        match prev_pw {
            Some(v) => std::env::set_var("PEDRA_WAL_PWRITE", v),
            None => std::env::remove_var("PEDRA_WAL_PWRITE"),
        }
        match prev_buf {
            Some(v) => std::env::set_var("PEDRA_WAL_BUFFER", v),
            None => std::env::remove_var("PEDRA_WAL_BUFFER"),
        }
        match prev_ag {
    ...
    /// RFC-0230: `take_pwrite_job` clone Err must reach the group leader as
    /// I/O failure — never `.ok().flatten()` into an empty `write_pending_frame`
    /// that still publishes.
    #[test]
    fn rfc0230_pwrite_clone_fail_does_not_publish() {
        let _axis = ENV_AXIS_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let prev_pw = std::env::var("PEDRA_WAL_PWRITE").ok();
        let prev_buf = std::env::var("PEDRA_WAL_BUFFER").ok();
        let prev_ag = std::env::var("PEDRA_ASYNC_GROUP").ok();
        ...
        db.set_write_group_catchup_window(Duration::from_millis(20));
        db.put(b"warm", b"1").unwrap();
        assert_eq!(db.get(b"warm").as_deref(), Some(&b"1"[..]));
        env.fail_pwrite
            .store(true, std::sync::atomic::Ordering::SeqCst);
        // RFC-0233: off-lock is Arc+pwrite (no dup). Inject write_all_at_shared.
        let n = 4usize;
        ...
        for i in 0..n {
            ...
            handles.push(thread::spawn(move || {
                barrier.wait();
                db.put(&[b'k', u8::try_from(i).expect("n fits u8")], b"v")
            }));
        }
        ...
        assert!(
            results.iter().any(|r| r
                .as_ref()
                .err()
                .is_some_and(|e| e.to_string().contains("pwrite failure"))),
            "at least one group member must see take_pwrite_job pwrite Err: {results:?}"
        );
        for (i, r) in results.iter().enumerate() {
            if r.is_err() {
                let k = [b'k', u8::try_from(i).expect("n fits u8")];
                assert_eq!(db.get(&k), None, "pwrite-fail member must not publish");
            }
        }
        restore_wal_pwrite_env(prev_pw, prev_buf, prev_ag);
        let _ = fs::remove_dir_all(&dir);
    }

    /// RFC-0230: `job.run` `write_all_at` Err after a successful clone is
    /// I/O failure (the group leader's `Ok(Some(job))` arm). Single-thread
    /// Wal so we do not sit on the write-group `recv`.
    #[test]
    fn rfc0230_pwrite_write_all_at_fail_is_err() {
        let _axis = ENV_AXIS_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let prev_pw = std::env::var("PEDRA_WAL_PWRITE").ok();
        let prev_buf = std::env::var("PEDRA_WAL_BUFFER").ok();
        let prev_ag = std::env::var("PEDRA_ASYNC_GROUP").ok();
        ...
        let mut w = crate::wal::Wal::create_on(&env, &path).unwrap();
        let ops = vec![crate::batch::WriteOp::put(
            1,
        ...
        let job = w
            .take_pwrite_job()
            .unwrap()
            .expect("clone ok; job must be armed so lead() hits job.run()");
        assert!(
            job.run().is_err(),
            "injected write_all_at on cloned fd must be Err, not Ok that publishes"
        );
        w.finish_pwrite(0, 0);
        restore_wal_pwrite_env(prev_pw, prev_buf, prev_ag);
        let _ = fs::remove_dir_all(&dir);
    ...
    /// RFC-0230: `write_all_at` Err on the cloned fd through the group
    /// leader (`take_pwrite_job` → `job.run`) must be I/O failure — never
    /// publish. Lone writers with `PEDRA_WAL_PWRITE=1` hit the same Env
    /// seam in `write_pending_frame`. Sticky `fail_pwrite`; n=4 barrier.
    #[test]
    fn rfc0230_pwrite_write_all_at_fail_does_not_publish() {
        let _axis = ENV_AXIS_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let prev_pw = std::env::var("PEDRA_WAL_PWRITE").ok();
        let prev_buf = std::env::var("PEDRA_WAL_BUFFER").ok();
        let prev_ag = std::env::var("PEDRA_ASYNC_GROUP").ok();
        ...
        assert!(
            ...
            "injected write_all_at through ConcurrentDb put must be Err: {results:?}"
        );
        for i in 0..n {
            let k = [b'k', u8::try_from(i).expect("n fits u8")];
            assert_eq!(db.get(&k), None, "must not publish after write_all_at fail");
        }
        restore_wal_pwrite_env(prev_pw, prev_buf, prev_ag);
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    /// RFC-0229 P0.2: ConcurrentDb parks that wake via the OS (not a World
    /// grant token) are the named inventory. A new production `sleep` /
    /// `wait_for` / `recv` is the Python TSV's job; this tooth names the
    /// four RFC sites on the rustc-linked file.
    #[test]
    fn rfc0229_os_park_sites_inventoried() {
        let src = include_str!("concurrent_kernel.rs");
        let prod = src
            .split("\nmod tests {")
            .next()
            .expect("production body before mod tests");
        assert!(
            prod.contains("fn await_flush_debt"),
            "flush-debt OS park must stay named"
        );
        assert!(
            prod.contains("fn await_l0_park"),
            "L0 stall OS park must stay named"
        );
        assert!(
            prod.contains("arrived.wait_for"),
            "write-group Condvar OS park must stay named"
        );
        assert!(
            prod.contains("write_group_wait_grant("),
            "RFC-0229 P1.1: production wait matches the grant token"
        );
        assert!(
            prod.contains("db.write()"),
            "RwLock write OS park must stay named"
        );
        assert!(
            !prod.contains("WorldGrantToken"),
            "P1.1 grant token is not this slice — a silent token would hide OS parks"
        );
    }
    ...
    struct FenceEnv {
        ...
        /// RFC-0230: `try_clone_handle` returns Err (pwrite job must not swallow).
        fail_clone: std::sync::Arc<std::sync::atomic::AtomicBool>,
        /// RFC-0230: sticky `write_all_at` Err on the cloned fd (`job.run`).
        fail_pwrite: std::sync::Arc<std::sync::atomic::AtomicBool>,
        /// RFC-0237: WAL I/O probe (group leader must have dropped Db write lock).
        wal_io_probe: std::sync::Arc<dyn Fn() + Send + Sync>,
    }
    ...
    impl FenceEnv {
        fn new() -> Self {
            Self {
                ...
                fail_clone: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false)),
                fail_pwrite: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false)),
                wal_io_probe: std::sync::Arc::new(|| {}),
            }
        }
    }
    ...
    impl crate::env::EnvFile for FenceFile {
        fn write_wal(&mut self, buf: &[u8], at: u64) -> std::io::Result<()> {
            (self.env.wal_io_probe)();
        ...
        fn positional_writes(&self) -> bool {
            true
        }
        fn try_clone_handle(&mut self) -> std::io::Result<Option<Self>> {
            if self
                .env
                .fail_clone
                .load(std::sync::atomic::Ordering::SeqCst)
            {
                return Err(std::io::Error::other("injected wal clone failure"));
            }
            Ok(Some(FenceFile {
        ...
        fn write_all_at(&mut self, buf: &[u8], at: u64) -> std::io::Result<()> {
            self.write_all_at_shared(buf, at)
        }
        fn write_wal_at_shared(&self, buf: &[u8], at: u64) -> std::io::Result<()> {
            self.write_all_at_shared(buf, at)
        }
        fn write_all_at_shared(&self, buf: &[u8], at: u64) -> std::io::Result<()> {
            ...
            if self
                ...
                .load(std::sync::atomic::Ordering::SeqCst)
            {
                return Err(std::io::Error::other("injected wal pwrite failure"));
            }
            #[cfg(unix)]
            {
                std::os::unix::fs::FileExt::write_all_at(&self.inner, buf, at)
            }
            #[cfg(not(unix))]
            {
                Err(std::io::Error::other("pwrite requires unix"))
            }
        }
        ...
        fn set_len_shared(&self, len: u64) -> std::io::Result<()> {
            std::fs::File::set_len(&self.inner, len)
        }
        fn release_wal_map(&self) {
            pedradb_posix::mapped_release(&self.inner);
        }
    }
    ...
    impl crate::env::Env for FenceEnv {
        ...
        fn open_rw(&self, path: &std::path::Path) -> std::io::Result<Self::File> {
            Ok(FenceFile {
    ...
    /// writers zipf-put; RFC-0233 P1.4 default keeps the bypass
    /// (`queued == 0`, per-writer batches). Every key is readable
    /// after the barrier.
    #[test]
    fn concurrent_async_one_op_overwrite_is_visible() {
        ...
        assert!(
            queued == 0,
            "default (RFC-0233 P1.4) 4-writer overwrite keeps the bypass"
        );
        assert!(
            batches == submits,
            "per-writer batches (batches={batches} submits={submits})"
        );
        for u in 0..records {
            let k = format!("c/{u:06}");
    ...
    /// RFC-0211 → RFC-0233 P1.4: the bypass is the default at writers ==
    /// ncpu 1-op async (`queued == 0`, per-writer batches).
    /// `PEDRA_RMW_SCHED=1` restores the merged group (drainable,
    /// amortized batches).
    #[test]
    fn rfc0211_env_axis_rmw_sched_forms_groups() {
        ...
        std::env::remove_var("PEDRA_RMW_SCHED");
        {
            ...
            assert_eq!(
                queued, 0,
                "default (RFC-0233 P1.4): writers == ncpu ({threads}) keep the bypass"
            );
            assert_eq!(
                batches, submits,
                "default (RFC-0233 P1.4): per-writer batches"
            );
            db.close().unwrap();
        }
        let db = ConcurrentDb::open(&dir1).unwrap();
