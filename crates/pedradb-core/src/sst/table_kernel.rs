...
#[must_use]
pub fn point_hash_probe_count() -> u64 {
    POINT_HASH_PROBES.load(std::sync::atomic::Ordering::Relaxed)
}

/// RFC-0235: zero point-hash probes (tests).
pub fn reset_point_hash_probe_count() {
    POINT_HASH_PROBES.store(0, std::sync::atomic::Ordering::Relaxed);
}

/// RFC-0236: in-block hash index probes (1-byte bucket → restart).
#[must_use]
pub fn block_hash_probe_count() -> u64 {
    BLOCK_HASH_PROBES.load(std::sync::atomic::Ordering::Relaxed)
}

/// RFC-0236: CONFLICT/miss fallbacks to linear scan.
#[must_use]
pub fn block_hash_fallback_count() -> u64 {
    BLOCK_HASH_FALLBACKS.load(std::sync::atomic::Ordering::Relaxed)
}

/// RFC-0236: zero in-block hash counters (tests).
pub fn reset_block_hash_probe_count() {
    BLOCK_HASH_PROBES.store(0, std::sync::atomic::Ordering::Relaxed);
    BLOCK_HASH_FALLBACKS.store(0, std::sync::atomic::Ordering::Relaxed);
}

const BLOCK_HASH_MAGIC: u32 = 0x0236_B0C4;
const BLOCK_HASH_FREE: u8 = 255;
const BLOCK_HASH_CONFLICT: u8 = 254;
const BLOCK_RESTART: usize = 16;
const ENTRY_TRUNC_FLAG: u32 = 0x8000_0000;

/// RFC-0236: shared prefix length (restart-head truncation).
#[must_use]
pub fn prefix_shared_len(a: &[u8], b: &[u8]) -> usize {
    a.iter().zip(b.iter()).take_while(|(x, y)| x == y).count()
}

/// Bytes for a full (untruncated) SST entry header+key+trailer (no value).
#[must_use]
pub fn prefix_full_key_bytes(uk_len: usize) -> usize {
    4usize.saturating_add(uk_len).saturating_add(8)
}

/// Bytes for a truncated SST entry header+shared+suffix+trailer (no value).
#[must_use]
pub fn prefix_trunc_key_bytes(uk_len: usize, shared: usize) -> usize {
    4usize
        .saturating_add(1)
        .saturating_add(uk_len.saturating_sub(shared))
        .saturating_add(8)
}
...
impl SstTable {
    ...
    fn blocks_for_point(&self, user_key: &[u8]) -> std::ops::Range<usize> {
        ...
        POINT_HASH_PROBES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let ge = self
            .index
...
fn seek_point_in_plain_block(
    ...
    if let Some(tr) = hash_tr {
        BLOCK_HASH_PROBES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
...
fn block_hash_lookup(trailer: &[u8], user_key: &[u8]) -> Option<BlockHashHit> {
    if trailer.len() < 6 {
        return None;
    }
    let n_buckets = u16::from_le_bytes(trailer[trailer.len() - 2..].try_into().ok()?) as usize;
    if 4 + n_buckets + 2 != trailer.len() {
        return None;
    }
    let buckets = &trailer[4..4 + n_buckets];
    let (h1, _) = crate::bloom::hash_pair(user_key);
    let b = (h1 as usize) % n_buckets;
    let v = *buckets.get(b)?;
    if v == BLOCK_HASH_FREE {
        return Some(BlockHashHit::Free);
    }
    if v == BLOCK_HASH_CONFLICT {
        return Some(BlockHashHit::Conflict);
    }
    Some(BlockHashHit::Restart(v as usize))
}

fn seek_point_from_restart(
    plain: &[u8],
    user_key: &[u8],
    snapshot: SequenceNumber,
    path: &Path,
...
fn consider_point(
    best: &mut Option<(SequenceNumber, Lookup)>,
    user_key: &[u8],
    uk: &[u8],
    trailer: &[u8],
    val_slice: &[u8],
    ...
    match uk.cmp(user_key) {
        ...
        Ordering::Equal => {
            ...
            packed.copy_from_slice(trailer);
            let (sequence, kind) =
                crate::key::unpack_sequence_and_type(u64::from_be_bytes(packed))?;
            if kind != ValueType::RangeDeletion && sequence <= snapshot {
                if crate::lookup_kernel::prefer_newer_seq(
                    ...
                    let look = match kind {
                        ValueType::Deletion => Lookup::Deleted,
                        ValueType::Value => Lookup::Found(Bytes::copy_from_slice(val_slice)),
                        ValueType::RangeDeletion => Lookup::NotFound,
                    };
...
fn seek_point_scan(
    ...
    while pos < plain.len() {
        ...
        pos += 4;
        let truncated = raw_len & ENTRY_TRUNC_FLAG != 0;
        let payload = (raw_len & !ENTRY_TRUNC_FLAG) as usize;
        let stop = if truncated {
            if payload < 9 {
                return Err(CoreError::Internal(format!(
                    "SST truncated key too short in {}",
                    path.display()
                )));
            }
            let Some(body_end) = pos.checked_add(payload) else {
                return Err(CoreError::Internal(format!(
                    "SST block entry truncated in {}",
                    path.display()
                )));
            };
            if body_end + 4 > plain.len() {
                return Err(CoreError::Internal(format!(
                    "SST block entry truncated in {}",
                    path.display()
                )));
            }
            let shared = plain[pos] as usize;
            if shared > restart_uk.len() {
                return Err(CoreError::Internal(format!(
                    "SST truncated shared past restart in {}",
                    path.display()
                )));
            }
            let suffix = &plain[pos + 1..body_end - 8];
            uk_buf.clear();
            ...
            let stop = entry_i >= skip_entries
                && consider_point(
                    &mut best,
                    user_key,
                    &uk_buf,
                    &plain[body_end - 8..body_end],
                    &plain[body_end + 4..val_end],
                    snapshot,
                )?;
            pos = val_end;
            stop
            ...
            let Some(ikey_end) = pos.checked_add(payload) else {
                return Err(CoreError::Internal(format!(
                    "SST block entry truncated in {}",
                    path.display()
                )));
        ...
        entry_i = entry_i.saturating_add(1);
    }
    Ok(best)
}
...
fn decode_block_bytes(
    ...
    let (entries_plain, _hash) = split_block_hash(&plain);
    let mut bc = Cursor::new(entries_plain);
    let mut entries = Vec::new();
    let mut restart_uk: Option<Bytes> = None;
    let mut i = 0usize;
    while !crate::write_admission_kernel::batch_is_empty(bc.data.len().saturating_sub(bc.pos) as u64)
    {
        let e = read_entry_maybe_trunc(&mut bc, restart_uk.as_deref())?;
        if i % BLOCK_RESTART == 0 {
            restart_uk = Some(e.0.user_key.clone());
        }
        i += 1;
        entries.push(e);
    }
...
fn rebuild_bloom(entries: &[(InternalKey, Bytes)]) -> BloomFilter {
    ...
    let nparts = crate::filter_partition_kernel::filter_nparts(entries.len() as u64);
    let mut bloom = BloomFilter::with_partitions(
        entries.len(),
        crate::bloom::bits_per_key_for_run(entries.len() as u64),
        nparts,
    );
...
fn read_entry(c: &mut Cursor<'_>) -> Result<(InternalKey, Bytes)> {
    read_entry_maybe_trunc(c, None)
}

fn read_entry_maybe_trunc(
    c: &mut Cursor<'_>,
    restart_uk: Option<&[u8]>,
) -> Result<(InternalKey, Bytes)> {
    let raw_len = c.read_u32()?;
    if raw_len & ENTRY_TRUNC_FLAG != 0 {
        let payload = (raw_len & !ENTRY_TRUNC_FLAG) as usize;
        let body = c.read_slice(payload)?;
        if body.is_empty() {
            return Err(CoreError::Internal("truncated SST key empty".into()));
        }
        let shared = body[0] as usize;
        if payload < 9 {
            return Err(CoreError::Internal("truncated SST key too short".into()));
        }
        let suffix = &body[1..payload - 8];
        let trailer = &body[payload - 8..];
        let rst = restart_uk.unwrap_or(&[]);
        if shared > rst.len() {
            return Err(CoreError::Internal("truncated SST shared past restart".into()));
        }
        let mut uk = Vec::with_capacity(shared + suffix.len());
        uk.extend_from_slice(&rst[..shared]);
        uk.extend_from_slice(suffix);
        let mut encoded = uk;
        encoded.extend_from_slice(trailer);
        let ikey = InternalKey::decode(&encoded)?;
        let val_len = c.read_u32()? as usize;
        let value = Bytes::copy_from_slice(c.read_slice(val_len)?);
        return Ok((ikey, value));
    }
    let ikey_len = raw_len as usize;
...
fn encode_entry_into_maybe_trunc(
    ikey: &InternalKey,
    value: &[u8],
    restart_uk: Option<&[u8]>,
    out: &mut Vec<u8>,
    ...
    if let Some(rst) = restart_uk {
        let shared = prefix_shared_len(rst, ikey.user_key.as_ref()).min(255);
        if shared >= 2 && shared < ikey.user_key.len() {
            let suffix = &ikey.user_key.as_ref()[shared..];
            let payload = 1usize.saturating_add(suffix.len()).saturating_add(8);
            let flag_len = ENTRY_TRUNC_FLAG
                | u32::try_from(payload)
                    .map_err(|_| CoreError::Internal("internal key too large for SST".into()))?;
            out.extend_from_slice(&flag_len.to_le_bytes());
            #[allow(clippy::cast_possible_truncation)]
            out.push(shared as u8);
            out.extend_from_slice(suffix);
            let packed = crate::key::pack_sequence_and_type(ikey.sequence, ikey.kind);
...
fn append_block_hash(buf: &mut Vec<u8>, keys: &[Bytes]) {
    if crate::write_admission_kernel::batch_is_empty(keys.len() as u64) {
        return;
    }
    let n_restarts = keys.len().div_ceil(BLOCK_RESTART);
    if n_restarts > 253 {
        return;
    }
    let n_buckets = (keys.len().saturating_mul(2)).clamp(8, 4096);
    let mut buckets = vec![BLOCK_HASH_FREE; n_buckets];
    for (i, k) in keys.iter().enumerate() {
        #[allow(clippy::cast_possible_truncation)]
        let restart = (i / BLOCK_RESTART) as u8;
        let (h1, _) = crate::bloom::hash_pair(k.as_ref());
        let b = (h1 as usize) % n_buckets;
        if buckets[b] == BLOCK_HASH_FREE {
            buckets[b] = restart;
        } else if buckets[b] != restart {
            buckets[b] = BLOCK_HASH_CONFLICT;
        }
    }
    buf.extend_from_slice(&BLOCK_HASH_MAGIC.to_le_bytes());
    buf.extend_from_slice(&buckets);
    let nb = u16::try_from(n_buckets).unwrap_or(u16::MAX);
    buf.extend_from_slice(&nb.to_le_bytes());
}

fn split_block_hash(plain: &[u8]) -> (&[u8], Option<&[u8]>) {
    if plain.len() < 6 {
        return (plain, None);
    }
    ...
    (plain[..start].as_ref(), Some(&plain[start..]))
}
...
fn write_sst_try_sorted_body(
    ...
    let cap = bloom_hint.min(BLOOM_CAP_MAX);
    ...
    for item in entries {
        ...
        if crate::write_admission_kernel::batch_is_empty(block_buf.len() as u64) {
            ...
            block_restart_uk = Some(ikey.user_key.clone());
            block_entry_i = 0;
        }
        let pre_len = block_buf.len();
        let restart = if block_entry_i % BLOCK_RESTART == 0 {
            None
        } else {
            block_restart_uk.as_deref()
        };
        encode_entry_into_maybe_trunc(&ikey, &value, restart, &mut block_buf)?;
        if !same_user
            ...
            && !crate::write_admission_kernel::batch_is_empty(pre_len as u64)
        {
            ...
            block_restart_uk = Some(ikey.user_key.clone());
            block_entry_i = 0;
            encode_entry_into_maybe_trunc(&ikey, &value, None, &mut block_buf)?;
        }
        block_user_keys.push(ikey.user_key.clone());
        if block_entry_i % BLOCK_RESTART == 0 {
            block_restart_uk = Some(ikey.user_key.clone());
        }
        block_entry_i = block_entry_i.saturating_add(1);
...
mod tests {
    ...
    fn sst_get_found_calls_visible_at() {
        ...
        let body = src
            ...
            .expect("SstTable::get");
        assert!(
            body.contains("get_live("),
            "SST get Found range-hide if must call get_live"
        );
        assert!(
    ...
    fn last_visible_under_prefix_with_calls_visible_at() {
        ...
        assert!(
            body.contains("get_live("),
            "last_visible Found range-hide if must call get_live"
        );
        assert!(
    ...
    fn rfc0236_point_get_uses_block_hash_or_falls_back() {
        ...
        for i in 0..24u32 {
            mem.put(
                ...
                1,
                Bytes::from_static(b"yyyyyyyyyyyy"),
            );
        }
        let table = write_sst(&path, &mem).unwrap();
        reset_block_hash_probe_count();
        let mut scratch = PointSeekScratch::default();
        let got = table
            .point_at_seeking(&0u32.to_be_bytes(), u64::MAX, &mut scratch)
            .unwrap();
        assert!(got.is_some(), "present key must be found");
        assert!(
            block_hash_probe_count() >= 1 || block_hash_fallback_count() >= 1,
            "point get must hit the in-block hash or its CONFLICT fallback, probes={} fallbacks={}",
            block_hash_probe_count(),
            block_hash_fallback_count()
        );
    ...
    fn write_sst_bloom_is_sized_by_written_keys_not_hint() {
        ...
        assert!(
            size < 4_000_000,
            "bloom must stay under the 2MiB-key cap (partitioned header included), not the 100M hint: {size} bytes"
        );
    ...
    fn evicted_payload_without_kit_fails_closed() {
        ...
        // Reload goes through the path (streaming bulk class).
        *table.payload.write() = crate::cache::ResidentBody::empty();
        let block = table.decode_block(0).expect("reload from path without kit");
        assert_eq!(block[0].0.user_key.as_ref(), b"k");
        let _ = std::fs::remove_file(&path);
    }
    ...
    fn compressed_repetitive_flush_passes_entry_count_check() {
        ...
        // Monkey small-run bloom (16 bpk) adds ~1.2 KiB vs uniform 10, so
        // the file no longer sits under `MIN_ENCODED_ENTRY` × 1200. It
        // must still beat the uncompressed 100-byte values (120 KiB).
        assert!(
            file_len < 1200 * 100,
            "compressed L0 must beat raw values: {file_len} bytes / 1200 entries"
        );
