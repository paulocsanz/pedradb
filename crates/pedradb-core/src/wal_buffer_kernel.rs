...
mod tests {
    ...
    fn wal_buffer_should_flush_at_threshold() {
        ...
        assert!(!should_flush(WAL_BUF_MAX_DEFAULT_BYTES - 1, WAL_BUF_MAX_DEFAULT_BYTES));
        assert!(should_flush(WAL_BUF_MAX_DEFAULT_BYTES, WAL_BUF_MAX_DEFAULT_BYTES));
    }
    ...
    fn wal_buffer_lone_always_flushes_group_respects_cap() {
        ...
        assert!(should_flush_for_workload(1, cap, true), "1c FlushWAL per Write");
        assert!(
