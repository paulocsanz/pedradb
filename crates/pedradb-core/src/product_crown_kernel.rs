...
mod tests {
    ...
    fn crown_as_is_breaks_on_unsynced_ack() {
        ...
        let s = WalState { acked: 64, synced: 0, written: 64 };
        assert!(!product_crown_as_is(&s), "spec leg refuses the unsynced ack");
        // The honest geometry on the same bytes passes both legs.
        let honest = WalState { acked: 64, synced: 64, written: 64 };
