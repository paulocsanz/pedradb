...
mod tests {
    ...
    fn history_segment_upload_restore_stay_on_walk() {
        ...
        for rel in [
            "src/history_kernel.rs",
            "src/db/lookup_archive_kernel.rs",
            "src/verify_kernel.rs",
    ...
    /// Restore prune stays on `sidecar_may_affect`. Do not make prune fail-closed.
    #[test]
    fn history_bloom_crc_mismatch_prune_stays_fail_open() {
        ...
        let crate_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
        let db = std::fs::read_to_string(crate_root.join("src/db/lookup_archive_kernel.rs"))
            .expect("lookup_archive_kernel.rs");
        assert!(
            db.contains("sidecar_may_affect"),
            "archive restore prune must stay on sidecar_may_affect"
        );
        let hist = std::fs::read_to_string(crate_root.join("src/history_kernel.rs"))
            .expect("history_kernel.rs");
        assert!(
    ...
    fn history_latest_put_readback_stays_rfc0092() {
        ...
        let hist = std::fs::read_to_string(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/history_kernel.rs"),
        )
        .expect("history_kernel.rs");
        assert!(
