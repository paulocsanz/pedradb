...
/// RFC-0227 P1.1 Isolated get walk: newest-first versions of one user key.
///
/// Index `0` is newest. First covering version decides via [`visible_at`]:
/// a live `Value` surfaces; `Deletion` / `RangeDeletion` / range-hidden
/// never; older versions do not resurrect. Empty walk is not live.
/// Index `while` (not `dyn Iterator`) so the Aeneas pin emits a `def`.
/// Production merge/SST/memtable/count_visible match this.
#[must_use]
pub fn get_live(kinds: &[ValueType], hiddens: &[bool]) -> bool {
    let n = kinds.len().min(hiddens.len());
    let mut i = 0usize;
    let mut live = false;
    let mut decided = false;
    while i < n {
        if !decided {
            live = visible_at(kinds[i], hiddens[i]);
            decided = true;
        }
        i += 1;
    }
...
/// AS-IS get walk: every version surfaces (hidden/Deletion leak).
#[must_use]
pub fn get_live_as_is(_kinds: &[ValueType], _hiddens: &[bool]) -> bool {
    true
}

/// RFC-0227 P1.3 D1: honest-Env put that the handler acked survives
/// crash→reopen→get. Composes `put_handler_plan` × `wal_commit_plan` ×
/// `crash_legal` × `recover_collect_act` × [`get_live`]. Lying Env suspends.
#[must_use]
pub fn put_crash_reopen_survives(
    n_records: u64,
    commit_failed: bool,
    need_sync: bool,
    sync_fail: bool,
    ...
    use crate::wal::recover_kernel::{recover_collect_act, RecoverAct, RecoverKind};
    use crate::write_admission_kernel::{
        put_handler_plan, wal_commit_plan, PutHandlerPlan, WalCommitPlan,
    };
    if !env_honest {
        return true;
    }
    match put_handler_plan(n_records, commit_failed) {
        PutHandlerPlan::EmptyOk => true,
        PutHandlerPlan::RestoreSeqOnCommitErr => false,
        PutHandlerPlan::CommitThenFlush => match wal_commit_plan(need_sync, sync_fail) {
            WalCommitPlan::AppendSyncFence => false,
            WalCommitPlan::AppendApplyOk | WalCommitPlan::AppendSyncApplyOk => {
                let m = CrashModel::of(written, synced);
                if !crash_legal(m, cut) {
                    return true;
                }
                let kept = recover_collect_act(RecoverKind::Record, n_records, false, 0, false)
                    == RecoverAct::KeepRecord;
                kept && get_live(kinds, hiddens)
            }
        },
    }
}

/// AS-IS D1: recovered live even after a fenced/failed sync.
#[must_use]
pub fn put_crash_reopen_survives_as_is(
    _n_records: u64,
    _commit_failed: bool,
    _need_sync: bool,
    _sync_fail: bool,
...
pub fn visible_range_limited(
    ...
    while let Some((ikey, value)) = iter.next() {
        ...
        let snapshot_live = get_live(&[ikey.kind], &[range_hidden]);
...
/// Type-decl witness: Aeneas used to refuse `Box<dyn Iterator>` on this
/// struct. Field read only — no vtable `next` (FnOpDynamic still refused).
#[must_use]
#[allow(dead_code)]
pub(crate) fn streaming_visible_iter_emitted(it: &StreamingVisibleIter<'_>) -> usize {
    it.emitted
}
...
mod tests {
    ...
    fn window_oracle(
        ...
        for (ikey, value) in all {
            ...
            out.push(WindowKv {
                key: ikey.user_key,
                value,
                snapshot_live: get_live(&[ikey.kind], &[hidden]),
            });
        }
        out
    }
    ...
    fn gc_merge_close_run_calls_visible_at() {
        ...
        assert!(
            ...
            "close_run keep_only_latest Value range-hide if must call get_live"
        );
        assert!(
    ...
    fn gc_compact_entries_calls_visible_at() {
        ...
        assert!(
            body.contains("get_live("),
            "gc_compact_entries Value range-hide if must call get_live"
        );
        assert!(
    ...
    #[test]
    fn get_live_walk_on_production_fn() {
        // RFC-0227 P1.1: Isolated walk the get/merge path matches.
        assert!(!get_live(&[], &[]), "empty walk is not live");
        assert!(get_live(&[ValueType::Value], &[false]));
        assert!(!get_live(&[ValueType::Value], &[true]));
        assert!(!get_live(&[ValueType::Deletion], &[false]));
        assert!(!get_live(&[ValueType::RangeDeletion], &[true]));
        assert!(
            !get_live(&[ValueType::Deletion, ValueType::Value], &[false, false]),
            "older Value must not resurrect past a newer Deletion"
        );
        assert!(
        ...
        let src = include_str!("merge_kernel.rs");
        assert!(
            src.contains("while i < n") && src.contains("i += 1"),
            "get_live must be an Isolated index while that advances"
        );
        assert!(
            src.contains("let snapshot_live = get_live("),
            "StreamingVisibleIter emit must match get_live"
        );
        let db = include_str!("db_kernel.rs");
        assert!(
            db.contains("crate::merge::get_live("),
            "count_visible / get path must match get_live"
        );
        let mem = include_str!("memtable_kernel.rs");
        assert!(
            mem.contains("crate::merge::get_live("),
            "memtable get_entry must match get_live"
        );
        assert_eq!(
            get_live(&[ValueType::Value], &[false]),
            visible_at(ValueType::Value, false)
        );
    }

    #[test]
    fn put_crash_reopen_survives_on_production_fn() {
        use crate::write_admission_kernel::{
            put_handler_plan, wal_commit_plan, PutHandlerPlan, WalCommitPlan,
        };
        let live = [ValueType::Value];
        let vis = [false];
        assert!(
            put_crash_reopen_survives(1, false, false, false, true, 8, 8, 8, &live, &vis),
            "acked async put, legal crash, KeepRecord, live get"
        );
        assert!(
            put_crash_reopen_survives(2, false, false, false, true, 8, 8, 8, &live, &vis),
            "prefix_n is the put's n_records (not a literal 1)"
        );
        assert!(
            !put_crash_reopen_survives(1, false, true, true, true, 8, 8, 8, &live, &vis),
            "fenced sync-fail does not recover live"
        );
        assert!(
            put_crash_reopen_survives(1, false, true, true, false, 8, 8, 8, &live, &vis),
            "Lying Env suspends D1"
        );
        assert!(!put_crash_reopen_survives(
            1, true, false, false, true, 8, 8, 8, &live, &vis
        ));
        ...
        assert!(
            prod.contains("put_handler_plan(")
                && prod.contains("crash_legal(")
                && prod.contains("recover_collect_act(")
                && prod.contains("get_live(kinds"),
            "D1 composer must call put_handler_plan × crash_legal × recover × get_live"
        );
        assert!(
            prod.contains("n_records, false, 0, false"),
            "recover prefix_n is the put's n_records, not a literal 1"
        );
        assert!(
            !prod.contains("Record, 1, false, 0, false"),
            "recover must not ignore the WAL with prefix_n = 1"
        );
    }
    ...
    fn visible_at_on_live_range_del_is_not_ok() {
        ...
        assert!(
            body.contains("get_live("),
            "count_visible must match get_live"
        );
        assert!(
