...
pub struct MemTable {
    ...
    tail: Vec<Version>,
    /// Highest-seq tail index per user key, **sharded by CF prefix** (bytes before
...
impl MemTable {
    ...
    ///
    /// The index stores the **highest seq** for the user key, not the last
    /// insert. Off-lock WAL apply can land an older seq after a newer one;
    /// last-insert would make `get` return the stale value (`tail_best`).
    /// Equal seq still replaces (same-seq spill).
    #[inline]
    fn shard_insert(
        s: &mut TailShard,
        ...
        user_key: &Bytes,
        tail: &[Version],
        new_seq: SequenceNumber,
        i: usize,
    ) {
        let keep_newer = |old_i: usize| {
            tail.get(old_i).is_some_and(|old| {
                crate::key::ikey_seq_cmp(old.key.sequence, new_seq) == Ordering::Less
            })
        };
        if user_key.len() <= 32 {
            ...
            if point_cf(pfx) {
                if s.point.get(&sk).copied().is_some_and(keep_newer) {
                    return;
                }
                s.point.insert(sk, i);
                Self::point_ord_btree_insert_at(ord_bt, ord_live, pfx, sk, i);
            } else {
                if s.short.get(&sk).copied().is_some_and(keep_newer) {
                    return;
                }
                s.short.insert(sk, i);
            }
        } else if s.long.get(user_key.as_ref()).copied().is_some_and(keep_newer) {
            return;
        } else {
    ...
    pub fn get_entry(
        ...
        kinds.push(v.key.kind);
        hiddens.push(
            v.key.kind == ValueType::Value
                && self.range_deleted(user_key, v.key.sequence, snapshot),
        );
        if let Some(vers) = self.map.get(user_key) {
            for older in vers.iter() {
                if older.key.sequence >= v.key.sequence || older.key.sequence > snapshot {
                    continue;
                }
                kinds.push(older.key.kind);
                hiddens.push(
                    older.key.kind == ValueType::Value
                        && self.range_deleted(user_key, older.key.sequence, snapshot),
                );
            }
        }
        let look = if crate::merge::get_live(&kinds, &hiddens) {
...
mod tests {
    ...
    /// RFC-0237: applying an older seq after a newer one (off-lock WAL
    /// completion order) must not hide the higher seq from `get`.
    #[test]
    fn rfc0237_tail_idx_keeps_higher_seq_on_late_older_insert() {
        ...
        assert_eq!(
            m.get(b"k", 2),
            Lookup::Found(Bytes::from_static(b"new")),
            "higher seq must win even when the older seq inserts last"
        );
        assert_eq!(
            m.get(b"k", 1),
