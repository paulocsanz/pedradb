...
pub trait EnvFile: Read + Write + Seek {
    ...
    /// Positional write of `buf` at `at` (pwrite-shaped). Default: portable
    /// seek → write_all → seek-back. Unix [`File`] overrides with
    /// [`FileExt::write_all_at`] (no cursor). RFC-0193 / RFC-0230 P0.3.
    ///
    /// # Errors
    /// Underlying I/O (short write, seek).
    fn write_all_at(&mut self, buf: &[u8], at: u64) -> io::Result<()> {
        let cur = self.stream_position()?;
        self.seek(SeekFrom::Start(at))?;
        let wrote = self.write_all(buf);
        let back = self.seek(SeekFrom::Start(cur));
        wrote?;
        back?;
        Ok(())
    ...
    /// Stateless pwrite on a **shared** handle (`&self`). Default unsupported:
    /// the seek dance needs exclusive cursor access. Unix [`File`] overrides
    /// with [`FileExt::write_all_at`]. RFC-0233: off-lock WAL write clones
    /// an `Arc`, not the fd (`dup(2)`).
    ///
    /// # Errors
    /// Underlying I/O, or [`io::ErrorKind::Unsupported`].
    fn write_all_at_shared(&self, _buf: &[u8], _at: u64) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "positional write requires EnvFile::positional_writes",
        ))
    }

    /// 1c WAL drain (`write_lone_frame`): default sequential `write_all`.
    /// Host `File` overrides with `mapped_pwrite`.
    ///
    /// # Errors
    /// Underlying I/O.
    ...
    /// Shared-handle WAL write. Default [`Self::write_all_at_shared`].
    /// Host `File` mmap is unix (`mapped_pwrite`); the WAL meta lock is
    /// not held. Mixing mmap-on-shared with cursor `write_all` lost
    /// Darwin `ycsb_f_mc4` — every Shared drain stays on this method.
    ///
    /// # Errors
    /// Underlying I/O.
    fn write_wal_at_shared(&self, buf: &[u8], at: u64) -> io::Result<()> {
        self.write_all_at_shared(buf, at)
    }

    /// Unmap a WAL mmap cache entry (no-op default). Called before
    /// truncating a padded map back to the logical frontier.
    fn release_wal_map(&self) {}

    /// `set_len` on a shared handle. Default unsupported.
    ///
    /// # Errors
    /// Underlying I/O, or [`io::ErrorKind::Unsupported`].
    fn set_len_shared(&self, _len: u64) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "shared set_len requires a host file",
        ))
    }

    /// `fdatasync` on a shared handle. Default unsupported.
    ///
    /// # Errors
    /// Underlying I/O, or [`io::ErrorKind::Unsupported`].
    fn sync_data_shared(&self) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "shared sync requires a host file",
        ))
    }

    /// `fsync` on a shared handle. Default unsupported.
    ///
    /// # Errors
    /// Underlying I/O, or [`io::ErrorKind::Unsupported`].
    fn sync_all_shared(&self) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "shared sync requires a host file",
        ))
    ...
    /// Strongest data barrier on a shared handle. Default: [`Self::sync_data_shared`].
    ///
    /// # Errors
    /// Underlying I/O.
    fn sync_data_strong_shared(&self) -> io::Result<()> {
        self.sync_data_shared()
    }

    /// Best-effort preallocate on a shared handle. Default no-op.
    ///
    /// # Errors
    /// Underlying I/O when the platform implements the reservation.
    fn preallocate_shared(&self, len: u64) -> io::Result<()> {
        let _ = len;
        Ok(())
    }

    /// Whether [`Self::write_all_at_shared`] is a stateless pwrite (safe on
    /// an `Arc` clone while another thread holds the WAL meta lock).
    /// Default false: the seek dance needs exclusive cursor access.
    #[must_use]
    fn positional_writes(&self) -> bool {
        false
    }

    /// Dup this handle for an off-lock positional write. Default `Ok(None)`
    /// (sim / DST / in-memory): caller keeps the write under the WAL lock.
    ///
    /// # Errors
    /// Underlying I/O when the platform can dup the fd.
    fn try_clone_handle(&mut self) -> io::Result<Option<Self>>
    where
        Self: Sized,
    {
        Ok(None)
...
pub trait Env: Clone {
    ...
    /// Open a file for read+write **without** `O_APPEND` (RFC-0233).
    /// Production WAL uses this so `pwrite` honors the ticket offset.
    /// Default: [`Self::open_append`] (sim / DST in-memory — no `O_APPEND`).
    ///
    /// # Errors
    /// Underlying I/O.
...
impl EnvFile for File {
    ...
    #[cfg(unix)]
    fn write_all_at(&mut self, buf: &[u8], at: u64) -> io::Result<()> {
        std::os::unix::fs::FileExt::write_all_at(self, buf, at)
    }

    #[cfg(unix)]
    fn write_all_at_shared(&self, buf: &[u8], at: u64) -> io::Result<()> {
        std::os::unix::fs::FileExt::write_all_at(self, buf, at)
    }

    fn write_wal(&mut self, buf: &[u8], at: u64) -> io::Result<()> {
        // MAP_SHARED memcpy at ticket `at` (1c FlushWAL and Exclusive
        // group drain). mmap does not move the File cursor; the write_all
        // fallback must seek or it restarts at 0.
        match pedradb_posix::mapped_pwrite(self, buf, at) {
            Ok(()) => Ok(()),
            Err(_) => {
                self.seek(SeekFrom::Start(at))?;
                self.write_all(buf)
    ...
    fn release_wal_map(&self) {
        pedradb_posix::mapped_release(self);
    }

    fn set_len_shared(&self, len: u64) -> io::Result<()> {
        File::set_len(self, len)
    }
...
impl EnvFile for std::io::Cursor<Vec<u8>> {
    fn sync_data(&mut self) -> io::Result<()> {
        Ok(())
    }
