...
#[must_use]
pub fn fit_write_family_curve(anchors: &[ScaleAnchor], bytes_per_entry: u64, warm_cap: u64) -> CurveFit {
    ...
    let base_num = sy.saturating_mul(b_den).saturating_sub(b_num.saturating_mul(sc));
...
pub fn rocks_cycle_ns(anchors: &[ScaleAnchor]) -> u64 {
    ...
    anchors.iter().map(|a| cycle_from_qps(a.rocks_qps)).sum::<u64>() / n
}
...
pub fn ratio_curve_table(
    ...
    let points = ratio_curve(fit, rocks_cycle, anchors, scales, SCALE_BYTES_PER_ENTRY, warm_cap);
    let cuts = points
        .iter()
    ...
    pub fn render(&self) -> String {
        ...
        for (pt, cut) in self.points.iter().zip(&self.cuts) {
            ...
            out.push_str(&format!(
                ...
                pt.records, pt.cold_permille, pt.cycle_hat_ns, pt.qps_hat, pt.ratio_hat_permille, measured
            ));
            if cut.deficit_ns > 0 {
...
mod tests {
    ...
    fn rfc0197_refit_at_wrong_ram_breaks_tolerance() {
        ...
        let mut p = LINUX_QUIET_0189_P01;
        p.mem_guard = 0;
        let t = ratio_curve_table(
            &WRITE_FAMILY_ANCHORS_2026_09_10,
            &SCALES,
            64 << 30,
            p,
        );
