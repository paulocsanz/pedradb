...
//!
//! Run: `cargo run -p pedradb-core --example secondary_index` is not wired;
//! use workspace example once registered, or:
//!   cargo run --example secondary_index -p pedradb-core
//!
//! Prefer: from repo root after adding [[example]] or:
//!   cargo run --manifest-path crates/pedradb-core/Cargo.toml --example secondary_index
