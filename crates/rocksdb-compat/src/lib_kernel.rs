...
impl<E: PedraEnv> DB<E> {
    ...
    #[must_use]
    pub fn workload_class_now(&self) -> pedradb_core::WorkloadClass {
        self.inner.workload_class_now()
    }

    /// RFC-0235 L0+L1 filter pins.
    #[must_use]
    pub fn hot_filter_pin_count(&self) -> u64 {
        self.inner.hot_filter_pin_count()
    }
    ...
    /// rust-rocksdb `wait_for_compact`. Drain parked mems to L0, then L0→L1
    /// and stacked-L1 repair until the tree is leveled. RFC-0234 P1.3: a
    /// 10M two-round seed restacks the keyspace; returning at the first
    /// L0=0 while parked tables still sit in memory lets the flush worker
    /// install leftover L0 into the timed window (2 tables/op on deps_scan).
    ///
    /// Lock order matches [`Self::flush`]: compact gate, then flush lock
    /// (inside `materialize_parked_once`). `compact_l0_once` re-takes the
    /// gate after parked drain so it does not nest.
    /// `timeout_us == 0` waits up to 600 s (10M L2→L3 drain of bulk files).
    ///
    /// # Errors
    /// Compact I/O, or [`ErrorKind::TimedOut`] if parked/L0 remain.
    pub fn wait_for_compact(&self, opts: &WaitForCompactOptions) -> Result<()> {
        let timeout = if opts.timeout_us == 0 {
            std::time::Duration::from_secs(600)
        } else {
            std::time::Duration::from_micros(opts.timeout_us)
        };
        ...
        while std::time::Instant::now() < deadline {
            ...
            match self.compact_l0_once() {
                Ok(true) => continue,
                Ok(false) => {}
                Err(e) => {
                    eprintln!("[pedra] wait_for_compact compact_l0_once: {e}");
                    std::thread::sleep(std::time::Duration::from_millis(20));
                    continue;
                }
            }
            // L0 empty / L1 disjoint: drain L1→L2 (two-round MVCC).
            {
                let _gate = self.compact_gate.lock();
                match self.inner.compact_l1_drain_off_lock() {
                    Ok(true) => continue,
                    Ok(false) => {}
                    Err(e) => {
                        eprintln!("[pedra] wait_for_compact l1_drain: {e}");
                        std::thread::sleep(std::time::Duration::from_millis(20));
                        continue;
                    }
                }
            }
            // Bulk 10M seed installs at MAX_LSM_LEVEL (3). Round 2 lands
            // L0→L1→L2. Drain L2→L3 merging overlapping bulk files so a
            // 25-key prefix overlaps 1 SST, not L2+L3.
            {
                let _gate = self.compact_gate.lock();
                match self.inner.compact_level_drain_off_lock(2) {
                    Ok(true) => continue,
                    Ok(false) => {}
                    Err(e) => {
                        eprintln!("[pedra] wait_for_compact l2_drain: {e}");
                        std::thread::sleep(std::time::Duration::from_millis(20));
                        continue;
                    }
                }
            ...
            {
                let _gate = self.compact_gate.lock();
                match self.inner.compact_stacked_level_off_lock(2) {
                    Ok(true) => continue,
                    Ok(false) => {}
                    Err(e) => {
                        eprintln!("[pedra] wait_for_compact l2_repair: {e}");
                        std::thread::sleep(std::time::Duration::from_millis(20));
                        continue;
                    }
                }
            ...
            {
                let _gate = self.compact_gate.lock();
                match self.inner.compact_stacked_level_off_lock(3) {
                    Ok(true) => continue,
                    Ok(false) => {}
                    Err(e) => {
                        eprintln!("[pedra] wait_for_compact l3_repair: {e}");
                        std::thread::sleep(std::time::Duration::from_millis(20));
                        continue;
                    }
                }
            }
            // One-key bulk SSTs stay disjoint after L2→L3 drain; pack
            // adjacent files under the split target so a 25-key prefix
            // overlaps 1 SST. Full-size 10M files are skipped.
            {
                let _gate = self.compact_gate.lock();
                match self.inner.compact_adjacent_coalesce_off_lock(3) {
                    Ok(true) => continue,
                    Ok(false) => {}
                    Err(e) => {
                        eprintln!("[pedra] wait_for_compact l3_coalesce: {e}");
                        std::thread::sleep(std::time::Duration::from_millis(20));
                        continue;
                    }
                }
            }
            let parked = self.inner.parked_unflushed_count();
            let l0 = self.read_probe().l0_files;
            let l1 = self.read_probe().level1_files;
            let l2 = self.inner.level_file_count(2);
            let l1_repair = self.inner.with_read(|d| d.l1_needs_repair());
            let l2_repair = self.inner.with_read(|d| d.level_needs_repair(2));
            let l3_repair = self.inner.with_read(|d| d.level_needs_repair(3));
            if parked == 0
                && l0 == 0
                && l1 == 0
                && l2 == 0
        ...
        let l2_repair = self.inner.with_read(|d| d.level_needs_repair(2));
        let l3_repair = self.inner.with_read(|d| d.level_needs_repair(3));
        if parked == 0 && l0 == 0 && l1 == 0 && l2 == 0 && !l1_repair && !l2_repair && !l3_repair {
            Ok(())
        } else {
            Err(Error {
                msg: format!(
                    "wait_for_compact timed out parked={parked} l0={l0} l1={l1} l2={l2} l1_repair={l1_repair} l2_repair={l2_repair} l3_repair={l3_repair}"
                ),
            ...
            loop {
                match rx.recv_timeout(wait) {
                    ...
                    Ok(CompactCmd::Run) | Err(RecvTimeoutError::Timeout) => {
                        ...
                        // RFC-0039 P2.2 / RFC-0234 P0.1: L0-at-trigger
                        // compact is a product path, not a settle pin.
...
fn compat_compact_once<E: PedraEnv>(inner: &ConcurrentDb<E>, gate: &Mutex<()>) -> bool {
    ...
    let job = inner.with_write(|db| {
        ...
        if pedradb_core::write_admission_kernel::batch_is_empty(db.level_file_count(0) as u64) {
            // RFC-0234 P1.3: unstack L1 after L0 is gone.
            db.prepare_stacked_l1_repair(opts).ok().flatten()
        } else {
...
mod tests {
    ...
    fn host_worker_drains_l0_at_trigger_without_idle() {
        ...
        assert!(
            max_l0 >= 1,
            "explicit flush must land L0 files (max L0={max_l0})"
        );
        // Leftover L0 drain when !inflight (RFC-0234 P1.3) may keep L0
        // below trigger between flushes — that is the product path, not a
        // miss. The busy-writer loop below still proves we do not wait
        // for the 200 ms idle window.
        let _ = saw_at_trigger;
    ...
    /// RFC-0234 P0.1: production ingest compacta L0 at trigger without the
    /// bench settle (`compact_l0_once`). Async puts + auto-flush park path
    /// (the 10M seed), not explicit `flush()`.
    #[test]
    fn rfc0234_l0_at_trigger_compacts() {
        let dir = tmp("rfc0234-l0-at-trigger");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        opts.set_write_buffer_size(4 * 1024);
        let db = DB::open(&opts, &dir).unwrap();
        ...
        assert!(
            flushes > 2,
            "need more than two flush-sized rounds, got {flushes}"
        );
        let deadline = Instant::now() + Duration::from_secs(8);
        let mut l0 = db.read_probe().l0_files;
        while Instant::now() < deadline {
            l0 = db.read_probe().l0_files;
            if l0 <= trigger {
                break;
            }
        ...
        assert!(
            l0 <= trigger,
            "production ingest must leave L0 <= trigger without bench settle, got {l0}"
        );
        let first = first.expect("put");
        assert_eq!(db.get(first).unwrap().as_deref(), Some(payload.as_slice()));
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// RFC-0234 P0.1: after a write-shaped put burst, the next letter
    /// (scan) sees `l0_files <= trigger` with no bench settle.
    #[test]
    fn rfc0234_ycsb_letter_does_not_inherit_l0() {
        let dir = tmp("rfc0234-letter-l0");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        opts.set_write_buffer_size(4 * 1024);
        let db = DB::open(&opts, &dir).unwrap();
        ...
        assert!(
            l0 <= trigger,
            "next letter must not inherit seed L0, got {l0} (no compact_l0_once / settle)"
        );
        let mut it = db.iterator(IteratorMode::Start).unwrap();
        let mut n = 0usize;
        while it.valid() {
        ...
        assert!(n >= 1, "scan letter must see the write burst");
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// RFC-0234 P1.3: after L0→L1 compact of a uniform keyspace, a short
    /// 25-key window overlaps **1** SST (range-partitioned L1, not stacked
    /// full-range runs).
    #[test]
    fn rfc0234_l1_range_partition_one_sst_for_short_scan() {
        let dir = tmp("rfc0234-l1-range");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        opts.set_write_buffer_size(4 * 1024);
        ...
        let mut jobs = 0usize;
        while db.compact_l0_once().unwrap() {
            jobs = jobs.saturating_add(1);
            assert!(jobs < 64, "L0/L1 drain did not converge");
        }
        assert_eq!(db.read_probe().l0_files, 0);
        assert!(
            db.read_probe().level1_files >= 2,
            "need range-split L1, got {}",
            db.read_probe().level1_files
        );
        let mid = keys.len() / 2;
        let start = keys[mid];
        let end = keys[mid + 24.min(keys.len() - 1 - mid)];
        let live = db.live_files().unwrap();
        let overlapping = live
            .iter()
            .filter(|f| f.level != 0)
            .filter(|f| {
                !f.start_key.is_empty()
                    && !f.end_key.is_empty()
                    && f.start_key.as_slice() <= end.as_slice()
                    && f.end_key.as_slice() >= start.as_slice()
            })
            .count();
        assert_eq!(
            overlapping,
            1,
            "short window must overlap 1 SST after range-partition L1, got {overlapping} (L1={})",
            db.read_probe().level1_files
        );
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    ...
    /// RFC-0234 P1.3: two sequential passes over the same keyspace (deps
    /// seed is 2 versions/record) restack L1 unless wait_for_compact drains
    /// parked mems, leftover L0, and overlapping L1. A 25-key window must
    /// overlap 1 SST — the unit test that only compact_l0_once-loops a
    /// one-round random seed is not this product path.
    #[test]
    fn rfc0234_two_round_seed_short_scan_one_sst() {
        let dir = tmp("rfc0234-two-round");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        opts.set_write_buffer_size(256 * 1024);
        opts.set_target_file_size_base(256 * 1024);
        let db = DB::open_cf_with_env(&opts, &dir, &[], IoUringEnv::default()).unwrap();
        db.inner.set_defer_auto_compact(true);
        let payload = vec![b'z'; 256];
        ...
        db.wait_for_compact(&WaitForCompactOptions::default())
            .unwrap_or_else(|e| {
                panic!(
                    "parked+L0+stacked-L1 drain must finish (L0={} L1={}): {e}",
                    db.read_probe().l0_files,
                    db.read_probe().level1_files
                )
            });
        assert_eq!(db.read_probe().l0_files, 0);
        assert!(
            !db.inner.with_read(|d| d.l1_needs_repair()),
            "wait_for_compact must unstack L1 (L1={})",
            db.read_probe().level1_files
        );
        let mid = keys.len() / 2;
        let start = keys[mid];
        ...
        assert_eq!(
            overlapping, 1,
            "two-round seed short window must overlap 1 SST after wait_for_compact, got {overlapping} (L0={} L1={})",
            db.read_probe().l0_files,
            db.read_probe().level1_files
        );
        drop(db);
    ...
    /// RFC-0234 P1.3: deps_scan keys are write-CF `ukey || ts` (two
    /// versions/record). The BE timestamp starts with NUL, so a default-raw
    /// put would tag each ukey as its own CF (`cf_family_of` splits on
    /// NUL) and never coalesce. Production seed encodes `write\0`+ukey.
    /// Sequential round 1 bulk-installs at MAX_LSM_LEVEL (3); round 2
    /// overlaps and goes L0→L1→L2. `wait_for_compact` must drain L2→L3
    /// merging those generations so `[ukey(u), ukey(u+25))` probes 1 SST.
    #[test]
    fn rfc0234_two_round_mvcc_short_scan_one_sst() {
        let dir = tmp("rfc0234-two-round-mvcc");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        opts.set_write_buffer_size(256 * 1024);
        opts.set_target_file_size_base(256 * 1024);
        let db = DB::open_cf_with_env(&opts, &dir, &["write"], IoUringEnv::default()).unwrap();
        db.inner.set_defer_auto_compact(true);
        let write = db.cf_handle("write").unwrap();
        let payload = vec![b'z'; 256];
        let n = 200u32;
        let ukey = |i: u32| format!("u/{i:06}").into_bytes();
        let mvcc = |i: u32, ts: u64| {
            let mut k = ukey(i);
        ...
        for round in 0..2u64 {
            for i in 0..n {
                db.put_cf(&write, mvcc(i, round * n as u64 + i as u64 + 1), &payload)
                    .unwrap();
            }
            db.flush().unwrap();
        }
        db.wait_for_compact(&WaitForCompactOptions::default())
            .unwrap_or_else(|e| {
                panic!(
                    "mvcc two-round drain must finish (L0={} L1={} L2={}): {e}",
                    db.read_probe().l0_files,
                    db.read_probe().level1_files,
                    db.inner.level_file_count(2)
                )
            });
        assert_eq!(db.read_probe().l0_files, 0);
        assert_eq!(
            db.inner.level_file_count(2),
            0,
            "L2 must drain into L3 (bulk MAX_LSM_LEVEL)"
        );
        let mid = n / 2;
        let start = ukey(mid);
        let end = ukey(mid + 25);
        db.reset_read_probe();
        let got = db
            .count_named("write", start.as_slice(), end.as_slice(), 25)
            .unwrap();
        assert!(got >= 1, "prefix count must see keys, got {got}");
        let p = db.read_probe();
        let live = db.live_files().unwrap();
        let summary: Vec<String> = live
            .iter()
            .take(12)
            .map(|f| {
                format!(
                    ...
                    String::from_utf8_lossy(&f.start_key),
                    String::from_utf8_lossy(&f.end_key)
                )
            })
            .collect();
        assert_eq!(
            p.scan_sst_probed, 1,
            "mvcc prefix count must probe 1 SST after L2→L3 drain, got {} (L0={} L1={} L2={} L3={} sst={} n={got} files={summary:?})",
            p.scan_sst_probed,
            p.l0_files,
            p.level1_files,
            db.inner.level_file_count(2),
            db.inner.level_file_count(3),
            p.sst_count
        );
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    ...
    /// RFC-0235 P0.1: a count-only window classifies as ShortRange.
    #[test]
    fn rfc0235_class_scan_window_is_short_range() {
        let dir = tmp("rfc0235-class-scan");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        let db = DB::open(&opts, &dir).unwrap();
        for i in 0..32u32 {
        ...
        db.reset_read_probe();
        for i in 0..16u32 {
            let start = i.to_be_bytes();
            let end = (i + 4).to_be_bytes();
            let _ = db.count_named("default", &start, &end, 8).unwrap();
        }
        assert_eq!(
            db.workload_class_now(),
            pedradb_core::WorkloadClass::ShortRange,
            "count-only window must be ShortRange"
        );
        assert_eq!(
            pedradb_core::workload_class_as_is(0, 0, 16, 0),
            pedradb_core::WorkloadClass::Mixed
        );
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// RFC-0235 P0.1: a get-hit window (compat LAST_GET) classifies as PointHit.
    #[test]
    fn rfc0235_class_get_window_is_point_hit() {
        let dir = tmp("rfc0235-class-hit");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        let db = DB::open(&opts, &dir).unwrap();
        db.put(b"k", b"v").unwrap();
        ...
        assert_eq!(
            db.workload_class_now(),
            pedradb_core::WorkloadClass::PointHit,
            "hit-only window must be PointHit (LAST_GET counts z1)"
        );
        assert_eq!(
            pedradb_core::workload_class_as_is(0, 16, 0, 0),
            pedradb_core::WorkloadClass::Mixed
        );
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    ...
    /// RFC-0235 P0.1: an overwrite-only window classifies as WriteBurst.
    #[test]
    fn rfc0235_class_overwrite_window_is_write_burst() {
        let dir = tmp("rfc0235-class-write");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        let db = DB::open(&opts, &dir).unwrap();
        db.reset_read_probe();
        for _ in 0..16u32 {
            db.put(b"hot", b"v").unwrap();
        }
        assert_eq!(
            db.workload_class_now(),
            pedradb_core::WorkloadClass::WriteBurst,
            "overwrite-only window must be WriteBurst"
        );
        assert_eq!(
            pedradb_core::workload_class_as_is(0, 0, 0, 16),
            pedradb_core::WorkloadClass::Mixed
        );
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    ...
    /// RFC-0235 P0.2: L0/L1 filter stays pinned; point gets do not re-decode bloom.
    #[test]
    fn rfc0235_point_get_does_not_reload_l0_filter() {
        let dir = tmp("rfc0235-pin-filter");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        opts.set_write_buffer_size(4 * 1024);
        let db = DB::open(&opts, &dir).unwrap();
        ...
        db.flush().unwrap();
        assert!(
            db.hot_filter_pin_count() >= 1,
            "L0 must pin its filter, got {}",
            db.hot_filter_pin_count()
        );
        let after_flush = pedradb_core::bloom_decode_count();
        for k in &keys {
            let _ = db.get(k).unwrap();
        }
        assert_eq!(
            pedradb_core::bloom_decode_count(),
            after_flush,
            "pinned L0 filter must not reload on point get"
        );
        db.reset_read_probe();
        let after_hits = pedradb_core::bloom_decode_count();
        for i in 0..32u32 {
            let miss = 0xFFFF_0000u32.wrapping_add(i).to_be_bytes();
            assert!(db.get(&miss).unwrap().is_none());
        }
        assert_eq!(
            db.workload_class_now(),
            pedradb_core::WorkloadClass::PointMiss,
            "miss-only window must be PointMiss"
        );
        assert_eq!(
            pedradb_core::bloom_decode_count(),
            after_hits,
            "pinned L0 filter must not reload on point miss"
        );
        let p = db.read_probe();
        eprintln!(
            "PHASE sst_probed get_sst_fallback={} get_mem_hit={} sst_count={} l0={} blocks_decoded={} bloom_decode={} point_hash={}",
            p.get_sst_fallback,
            p.get_mem_hit,
            p.sst_count,
            p.l0_files,
            p.blocks_decoded,
            pedradb_core::bloom_decode_count(),
            pedradb_core::point_hash_probe_count()
        );
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// RFC-0235 P1.1: get does not take three LSM locks.
    #[test]
    fn rfc0235_get_does_not_take_three_locks() {
        let dir = tmp("rfc0235-sv-locks");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        let db = DB::open(&opts, &dir).unwrap();
        db.put(b"k", b"v").unwrap();
        ...
        let n = pedradb_core::concurrent::get_lsm_lock_count();
        assert!(
            n >= 1,
            "get must enter ConcurrentDb LSM path (note_lsm_lock), got {n}"
        );
        assert!(n < 3, "get must not take three LSM locks, got {n}");
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// RFC-0235 P1.2: WriteBurst ingest does not grow L0 past the trigger.
    #[test]
    fn rfc0235_write_burst_does_not_grow_l0_past_trigger() {
        let dir = tmp("rfc0235-write-burst-l0");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        opts.set_write_buffer_size(4 * 1024);
        let db = DB::open_cf_with_env(&opts, &dir, &[], IoUringEnv::default()).unwrap();
        let payload = vec![b'z'; 512];
        let mut x = 0x51ed_c0ffu32;
        let mut max_l0 = 0usize;
        for _ in 0..48u32 {
            x = x.wrapping_mul(1664525).wrapping_add(1013904223);
            db.put(x.to_be_bytes(), &payload).unwrap();
            max_l0 = max_l0.max(db.read_probe().l0_files);
        }
        assert!(
            max_l0 <= pedradb_core::L0_COMPACTION_TRIGGER.saturating_add(1),
            "WriteBurst must not pile L0, max {max_l0}"
        );
        assert!(
            db.read_probe().l0_files <= pedradb_core::L0_COMPACTION_TRIGGER,
            "WriteBurst must drain L0 to trigger, got {}",
            db.read_probe().l0_files
        );
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    ...
    /// RFC-0236 P0.1: a point miss loads one filter partition, not the whole Bloom.
    #[test]
    fn rfc0236_point_miss_loads_one_filter_partition() {
        let dir = tmp("rfc0236-part-filter");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        let db = DB::open(&opts, &dir).unwrap();
        let payload = vec![b'x'; 32];
        // Even keys present; odd keys sit inside the SST envelope so a miss
        // still probes the partitioned filter (not the outside-bounds skip).
        for i in 0..32u32 {
            if i % 2 == 0 {
                db.put(i.to_be_bytes(), &payload).unwrap();
            }
        ...
        pedradb_core::reset_filter_part_load_count();
        let mut loaded = 0u32;
        for i in 1u32..32 {
            if i % 2 == 0 {
                continue;
            }
            let miss = i.to_be_bytes();
            let (h1, _) = pedradb_core::bloom::hash_pair(&miss);
            if pedradb_core::filter_partition(h1, 4) != 0 {
                continue;
            }
            assert!(db.get(&miss).unwrap().is_none());
        ...
        assert!(loaded >= 1, "need at least one in-range partition-0 miss");
        assert_eq!(
            pedradb_core::filter_part_load_count(),
            1,
            "same-partition misses must load one filter partition"
        );
        let p = db.read_probe();
        eprintln!(
            "PHASE filter_bytes={} sst_probed={} filter_part_load_count={} get_sst_fallback={}",
            pedradb_core::filter_bytes_loaded(),
            p.latest_sst_probed,
            pedradb_core::filter_part_load_count(),
            p.get_sst_fallback,
        );
        assert_eq!(
            pedradb_core::filter_partition_as_is(99, 4),
            0,
            "AS-IS twin is a single unpartitioned filter"
        );
        drop(db);
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// RFC-0236 P1.1: point get probes the in-block hash (or falls back).
    #[test]
    fn rfc0236_point_get_uses_block_hash_or_falls_back() {
        let dir = tmp("rfc0236-block-hash");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        let db = DB::open(&opts, &dir).unwrap();
    ...
    /// RFC-0236 P1.2: prefix truncation shrinks restart-following keys.
    #[test]
    fn rfc0236_prefix_truncation_shrinks_block() {
        assert!(
            pedradb_core::prefix_trunc_key_bytes(16, 8) < pedradb_core::prefix_full_key_bytes(16),
            "shared>=2 must encode smaller than a full key"
        );
        assert_eq!(pedradb_core::prefix_shared_len(b"abcdef", b"abcxyz"), 3);
        let dir = tmp("rfc0236-prefix-trunc");
        let mut opts = Options::new();
        opts.create_if_missing(true);
        let db = DB::open(&opts, &dir).unwrap();
        for i in 0..8u8 {
            let mut k = b"prefix/key-".to_vec();
            k.push(b'0' + i);
            db.put(&k, b"v").unwrap();
        }
        db.flush().unwrap();
