...
//! (`scripts/aeneas_commit.sh`). A Verus stand-in of these u64 watermarks
...
//! `pedradb-store::commit_kernel` (`catalog` `commit_raft_store`).
...
mod tests {
    ...
    #[test]
    fn commit_kernel_has_no_verus_cartoon() {
        let src = include_str!("commit_kernel.rs");
        let block = concat!("verus", "!", " {");
        let cfg = concat!("cfg(", "verus", "_keep", "_ghost)");
        assert!(!src.contains(block), "stand-in is not last-wins of rustc recover_commit");
    ...
    fn theorem_recover_and_commit_on_finite_domain() {
        ...
        for idx in 0..B {
            for commit in 0..B {
                ...
                if commit < idx {
                    assert_ne!(
                        propose_ack_ok(idx, commit),
                        propose_ack_ok_as_is(idx, commit)
                    );
                }
                n += 1;
            }
