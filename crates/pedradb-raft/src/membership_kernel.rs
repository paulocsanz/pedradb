...
//! (`scripts/aeneas_membership.sh`). A Verus stand-in of joint quorum
//! (including Seq vs `&[u64]`) is a model twin — not last-wins (deleted).
//!
...
mod tests {
    ...
    #[test]
    fn membership_kernel_has_no_verus_cartoon() {
        let src = include_str!("membership_kernel.rs");
        let block = concat!("verus", "!", " {");
        let cfg = concat!("cfg(", "verus", "_keep", "_ghost)");
        assert!(!src.contains(block), "stand-in is not last-wins of rustc joint_election_ok");
