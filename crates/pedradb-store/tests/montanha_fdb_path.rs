...
fn canary_lease_multi_node_exclusive() {
    ...
    {
        let mut c =
            StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_5E01)).unwrap();
    ...
    {
        let mut c =
            StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_5E02)).unwrap();
...
fn canary_index_batch_multi_node() {
    ...
    {
        let mut c =
            StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_1D01)).unwrap();
        c.elect_all(100).unwrap();
    ...
    {
        let mut c =
            StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_1D02)).unwrap();
...
fn canary_journal_seq_and_replica_lag() {
    let dir = temp();
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_FE01)).unwrap();
    ...
    drop(c);
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_FE02)).unwrap();
    c.elect_all(80).unwrap();
    let mut silent_wrong = 0u64;
    for i in 0..20u8 {
...
fn fast_replica_may_serve_local_applied() {
    let dir = temp();
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_F451)).unwrap();
    c.elect_all(80).unwrap();
...
fn multiwrite_multi_range_many_leaders() {
    let dir = temp();
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 4, SeedRng::new(0x17CA_AF01)).unwrap();
...
fn cluster_dst_lossy_net_i_maj_holds() {
    ...
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_D571)).unwrap();
    c.set_rpc_mode(RpcMode::Queued);
    for _ in 0..200 {
        c.tick().unwrap();
...
fn membership_remove_add_catchup() {
    let dir = temp();
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_AE11)).unwrap();
    c.elect_all(100).unwrap();
...
fn lagging_partition_heals_and_reads() {
    let dir = temp();
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_1A61)).unwrap();
    c.set_rpc_mode(RpcMode::Queued);
...
fn p21_rolling_restart_majority_holds() {
    let dir = temp();
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_0211)).unwrap();
    c.set_rpc_mode(RpcMode::Queued);
    elect_queued(&mut c, 120);
...
fn p21_clock_skew_advance_time_still_maj() {
    let dir = temp();
    let mut c =
        StoreCluster::open_with_rng_lab_direct(&dir, 3, 1, SeedRng::new(0x17CA_C10C)).unwrap();
    c.set_rpc_mode(RpcMode::Queued);
    elect_queued(&mut c, 100);
